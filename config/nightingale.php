...
return [
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | Product foundation namespace
    |--------------------------------------------------------------------------
    |
    | Salvaged from the independent-foundation effort (PR #92). These values
    | reserve a product-owned namespace and record a held candidate. They are
    | deliberately code-owned rather than environment toggles: configuration
    | alone must not be able to register a route, resolve an identity, query an
    | inpatient source, disclose patient data, or enable a native network
    | client.
    |
    | They describe the *next* boundary, not the one currently serving. The
    | live surface above (features, token, messaging) is unaffected.
    |
    */
    'status' => 'foundation',
    ...
    // Five independent, conjunctive preconditions evaluated by
    // App\Nightingale\Activation\NightingaleActivationGate. Each must hold on
    // its own; none can compensate for another.
    'activation' => [
