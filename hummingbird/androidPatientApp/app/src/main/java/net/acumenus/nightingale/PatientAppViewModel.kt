...
import net.acumenus.nightingale.data.PatientApiException
import net.acumenus.nightingale.data.PatientEnrollmentRequest
import net.acumenus.nightingale.data.PatientMessageAmendmentAction
import net.acumenus.nightingale.data.PatientPreferencesUpdate
...
internal class PatientAppViewModel(
            ...
            ready.synthetic || !apiEnabled || coordinator == null -> {
                state = state.copy(
                    deviceSessions = PatientDeviceSessionsState.Unavailable(
                        message = "Manage devices is not available right now. You can keep using Nightingale.",
                        canRetry = false,
                    ),
                )
            }
                ...
                if (error.statusCode == 401) {
                    ...
                    updateDeviceSessionFailure(
                        if (revocationConfirmed) {
                            ...
                            "Nightingale could not confirm that device was signed out. Review the list before trying again."
                        },
                    )
                }
            } catch (_: Exception) {
        ...
        state = state.copy(
            deviceSessions = PatientDeviceSessionsState.Unavailable(
                message = if (featureDisabled) {
                    "Manage devices is not available right now. You can keep using Nightingale."
                } else {
    ...
    private fun genericMessagingFailure() {
        updateMessaging { current ->
            current.copy(
                operation = PatientMessagingOperation.Failure(
                    "Nightingale could not confirm this request. Refresh before trying again; use your call button for immediate help.",
                ),
            )
        }
    }
    ...
    private fun List<net.acumenus.nightingale.data.PatientMessageThread>.replaceSummary(
        updated: net.acumenus.nightingale.data.PatientMessageThread,
    ): List<net.acumenus.nightingale.data.PatientMessageThread> =
    ...
    private fun Exception.patientMessage(): String = when (this) {
        is PatientApiException -> when (statusCode) {
            ...
            else -> "Nightingale could not load your information securely. Try again."
        }
        else -> "Nightingale could not connect securely. Check your connection and try again."
    }
