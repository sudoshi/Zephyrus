package net.acumenus.nightingale.ui
...
import net.acumenus.nightingale.PatientMessagingState
...
internal fun PatientExperienceScreen(
    ...
    onAmendMessage: (String, net.acumenus.nightingale.data.PatientMessageAmendmentAction, String?) -> Unit,
    ...
    val context = snapshot.contexts[selectedDestination] ?: PatientDataContext(
        ...
        sourceLabel = if (selectedDestination == PatientDestination.MESSAGES) {
            "Source: Nightingale communication service"
        } else {
...
private fun PatientPresentationPreferenceNotice(modifier: Modifier = Modifier) {
    ...
    GuidanceCard(
        ...
        body = "Nightingale is using ${choices.joinToString()}. Your device accessibility settings can make text larger. These display choices do not change your care plan, clinical orders, or urgent-help instructions.",
    ...
    if (snapshot.todayItems.isEmpty()) {
        item {
            GuidanceCard(
                ...
                body = "Ask your care team for the current plan. Nightingale only shows information that has been released for you.",
                modifier = Modifier.padding(horizontal = 16.dp),
            )
        }
    }
