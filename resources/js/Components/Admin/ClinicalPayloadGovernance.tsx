...
export default function ClinicalPayloadGovernance({
  ...
  if (!actionable || sourceId === null) {
    return <div className="rounded-md border border-dashed border-healthcare-border p-5 text-sm text-healthcare-text-secondary dark:border-healthcare-border-dark">
  ...
  return <div className="space-y-4">
      ...
      <table className="min-w-full divide-y divide-healthcare-border text-sm dark:divide-healthcare-border-dark">
        ...
        <tbody className="divide-y divide-healthcare-border dark:divide-healthcare-border-dark">
          {objects.map((object) => <tr key={object.uuid}>
            <td className="px-3 py-3"><div className="font-mono text-xs font-semibold">#{object.id} · {object.uuid.slice(0, 13)}…</div><div className="mt-1 text-xs text-healthcare-text-secondary">{readable(object.kind)} · {readable(object.classification)}</div></td>
            <td className="px-3 py-3"><div className="font-semibold">{readable(object.status)}</div><div className="mt-1 text-xs text-healthcare-text-secondary">{object.legalHold ? 'Legal hold active' : 'No legal hold'} · verified {time(object.lastVerifiedAt)}</div></td>
            <td className="px-3 py-3"><div>{time(object.retainUntil)}</div><div className="mt-1 text-xs text-healthcare-text-secondary">{object.deletionBlockers.length ? `Blocked: ${object.deletionBlockers.map(readable).join(', ')}` : 'No deletion dependencies'}</div></td>
          ...
          {objects.length === 0 ? <tr><td colSpan={4} className="px-3 py-5 text-center text-healthcare-text-secondary">No non-deleted payload authorities in this source.</td></tr> : null}
        </tbody>
      </table>
    </div>
    ...
    <div className="space-y-2">
      ...
      {quarantines.map((quarantine) => <div key={quarantine.uuid} className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-healthcare-border p-3 dark:border-healthcare-border-dark">
        <div><div className="font-mono text-xs font-semibold">Quarantine #{quarantine.id} · object #{quarantine.objectId}</div><div className="mt-1 text-xs text-healthcare-text-secondary">{readable(quarantine.category)} · {quarantine.reasonCode} · opened {time(quarantine.openedAt)}</div><div className="mt-1 text-xs text-healthcare-text-secondary">{quarantine.legalHold ? 'Legal hold blocks purge' : quarantine.deletionBlockers.length ? `Dependencies: ${quarantine.deletionBlockers.map(readable).join(', ')}` : 'No deletion dependencies'}</div></div>
      ...
      {quarantines.length === 0 ? <p className="rounded-md border border-dashed border-healthcare-border p-4 text-sm text-healthcare-text-secondary dark:border-healthcare-border-dark">No open quarantine authority in this source.</p> : null}
    </div>
    ...
    {pending ? <form onSubmit={submitRequest} className="space-y-3 rounded-md border border-healthcare-warning/40 bg-healthcare-warning/5 p-4">
      <div className="flex items-start justify-between gap-2"><div><h3 className="font-semibold text-healthcare-text-primary dark:text-healthcare-text-primary-dark">{requestLabel(pending.kind)}</h3><p className="mt-1 font-mono text-xs text-healthcare-text-secondary">Exact target #{pending.targetId} · {pending.targetUuid}</p></div><button type="button" className={secondaryButton} aria-label="Close governed request" onClick={() => setPending(null)}><X className="size-4" /></button></div>
      {(pending.kind === 'apply_hold' || pending.kind === 'release_hold') ? <label className="block text-xs font-semibold text-healthcare-text-secondary">Hold reason code<input required pattern="[a-z][a-z0-9._-]{2,119}" value={holdReasonCode} onChange={(event) => setHoldReasonCode(event.target.value)} className={`mt-1 ${inputClass}`} /></label> : null}
      <label className="block text-xs font-semibold text-healthcare-text-secondary">Governed rationale<textarea required minLength={10} maxLength={500} value={reason} onChange={(event) => setReason(event.target.value)} className={`mt-1 min-h-20 ${inputClass}`} placeholder="10–500 characters; do not enter PHI, clinical content, keys, paths, or credentials" /></label>
      ...
      {changes.map((change) => {
        ...
        return <div key={change.uuid} className="rounded-md border border-healthcare-border p-3 dark:border-healthcare-border-dark">
          ...
          {change.status === 'pending_approval' && canApprove ? currentUserId === change.authorUserId
            ...
            : <div className="mt-3 grid gap-2 sm:grid-cols-[1fr_auto_auto] sm:items-end"><label className="text-xs font-semibold text-healthcare-text-secondary">Independent rationale<input minLength={10} maxLength={500} value={rationale} onChange={(event) => setDecisionReasons({ ...decisionReasons, [change.uuid]: event.target.value })} className={`mt-1 ${inputClass}`} placeholder="10–500 characters; no PHI" /></label><button type="button" className={secondaryButton} disabled={busy === `decision:${change.uuid}` || rationale.trim().length < 10} onClick={() => decide(change, 'approved')}>Approve</button><button type="button" className={destructiveButton} disabled={busy === `decision:${change.uuid}` || rationale.trim().length < 10} onClick={() => decide(change, 'rejected')}>Reject</button></div>
            : null}
        </div>;
      })}
      {changes.length === 0 ? <p className="rounded-md border border-dashed border-healthcare-border p-4 text-sm text-healthcare-text-secondary dark:border-healthcare-border-dark">No clinical-payload governed change is recorded for this exact source.</p> : null}
    </div>
  </div>;
}
