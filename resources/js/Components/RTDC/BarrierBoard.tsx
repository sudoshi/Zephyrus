...
export function BarrierBoard({ barriers, onResolve }: BarrierBoardProps) {
  ...
  return (
    <ul className="flex flex-col gap-2">
      {barriers.map((b) => (
        <li key={b.barrier_id} className="flex items-center justify-between rounded-md bg-healthcare-background dark:bg-healthcare-background-dark p-3">
          <span className="flex items-center gap-2">
            ...
            <span className="text-healthcare-text-secondary dark:text-healthcare-text-secondary-dark">{b.description}</span>
          </span>
