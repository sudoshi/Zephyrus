...
export function BedNeedReadout({ bedNeed, capacityNow, demandExpected }: BedNeedReadoutProps) {
  ...
  const tone = deficit ? 'text-[var(--critical)]' : surplus ? 'text-[var(--success)]' : 'text-healthcare-text-secondary dark:text-healthcare-text-secondary-dark';
  ...
  return (
    <div className="rounded-lg bg-healthcare-surface dark:bg-healthcare-surface-dark p-5">
      ...
      <div className="text-caption mt-2 italic">For huddle decision — not an automated action.</div>
    </div>
  );
}
