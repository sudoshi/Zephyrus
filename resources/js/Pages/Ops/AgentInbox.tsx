      ...
      <div className="flex items-start justify-between gap-2">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[14px]/[19px] font-semibold text-healthcare-text-primary dark:text-healthcare-text-primary-dark">{definition.label}</span>
            {definition.readOnly && <ToneBadge tone="info">read-only</ToneBadge>}
          </div>
          <p className="mt-1 text-[12px]/[16px] text-healthcare-text-secondary dark:text-healthcare-text-secondary-dark">{definition.description}</p>
          <div className="mt-2 flex flex-wrap gap-1.5">
            {definition.toolAllowlist.map((tool) => (
              <span key={tool} className="rounded-md bg-slate-100 px-1.5 py-0.5 text-[10px]/[14px] font-medium text-slate-600 dark:bg-white/5 dark:text-slate-300">
                {tool}
              </span>
            ))}
          </div>
        ...
        {RUNNABLE.has(definition.key) && (
          <button
            ...
            className="inline-flex items-center gap-1 rounded-md bg-healthcare-primary px-2.5 py-1 text-[12px]/[16px] font-semibold text-white hover:opacity-90 disabled:opacity-60"
          >
        ...
        <div className="mt-3 space-y-2 rounded-md border border-healthcare-border p-3 dark:border-healthcare-border-dark">
          <div className="flex items-center gap-2">
            ...
            {result.output.headline && (
              <span className="text-[12px]/[16px] text-healthcare-text-secondary dark:text-healthcare-text-secondary-dark">{result.output.headline}</span>
            )}
          </div>
  ...
  return (
    <DashboardLayout>
      ...
      <PageContentLayout
        ...
        headerContent={
          <Link href="/ops/executive-brief" className="inline-flex items-center gap-1.5 rounded-md border border-healthcare-border px-3 py-1.5 text-[13px]/[18px] font-semibold text-healthcare-text-secondary hover:bg-slate-50 dark:border-healthcare-border-dark dark:text-healthcare-text-secondary-dark dark:hover:bg-white/5">
        ...
        <div className="space-y-4">
          ...
          <Panel title="Governed agents" icon={<Bot className="size-5 text-healthcare-primary" />}>
            {definitions.isLoading ? (
              <div className="text-[13px]/[18px] text-healthcare-text-secondary dark:text-healthcare-text-secondary-dark">Loading agents...</div>
            ) : (
          ...
          <Panel title="Pending approvals" icon={<ShieldCheck className="size-5 text-healthcare-primary" />}>
            {(inbox.data?.approvals.length ?? 0) === 0 ? (
              <div className="text-[13px]/[18px] text-healthcare-text-secondary dark:text-healthcare-text-secondary-dark">
                  ...
                  <li key={approval.approvalId} className="flex items-center justify-between gap-2 rounded-md border border-healthcare-border p-2.5 dark:border-healthcare-border-dark">
                    <div>
                      ...
                      {approval.action?.ownerName && (
                        <p className="text-[12px]/[16px] text-healthcare-text-secondary dark:text-healthcare-text-secondary-dark">Owner: {approval.action.ownerName}</p>
                      )}
                    </div>
          ...
          <Panel title="Active actions" icon={<Inbox className="size-5 text-healthcare-primary" />}>
            {(inbox.data?.actions.length ?? 0) === 0 ? (
              <div className="text-[13px]/[18px] text-healthcare-text-secondary dark:text-healthcare-text-secondary-dark">No active actions.</div>
            ) : (
              <ul className="space-y-2">
