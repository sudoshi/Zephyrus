...
function ScopeBadge({ mode }: { mode: CapabilityRow['scopeMode'] }) {
  ...
  return (
    <span className="inline-flex rounded-md border border-healthcare-border bg-healthcare-surface-secondary px-2 py-0.5 text-xs font-medium text-healthcare-text-secondary dark:border-healthcare-border-dark dark:bg-healthcare-surface-hover-dark dark:text-healthcare-text-secondary-dark">
      {labels[mode]}
    </span>
  );
}
          ...
          <div className="grid gap-3 lg:grid-cols-2">
            <section className="rounded-md border border-healthcare-border bg-healthcare-surface p-4 shadow-sm dark:border-healthcare-border-dark dark:bg-healthcare-surface-dark">
              ...
              <dl className="mt-4 space-y-3">
                <div>
                  <dt className="text-xs font-medium uppercase text-healthcare-text-secondary">Effective roles</dt>
                ...
                <div>
                  <dt className="text-xs font-medium uppercase text-healthcare-text-secondary">Effective capabilities</dt>
                  <dd className="mt-1 flex max-h-40 flex-wrap gap-1.5 overflow-y-auto">{currentPrincipal.capabilities.map((capability) => <code key={capability} className="rounded bg-healthcare-surface-secondary px-1.5 py-1 text-xs text-healthcare-text-primary dark:bg-healthcare-surface-hover-dark dark:text-healthcare-text-primary-dark">{capability}</code>)}</dd>
                </div>
              </dl>
            </section>
                  ...
                  {roles.map((role) => (
                    <tr key={role.role}>
                      ...
                      <td className="min-w-[36rem] px-3 py-2 align-top">
                        ...
                        <p className="mt-1 text-xs text-healthcare-text-secondary">{role.capabilityCount} {role.capabilityCount === 1 ? 'capability' : 'capabilities'}</p>
                      </td>
                    </tr>
                  ))}
                </tbody>
                ...
                <tbody className="divide-y divide-healthcare-border dark:divide-healthcare-border-dark">
                  {capabilities.map((capability) => (
                    <tr key={capability.capability}>
                      <td className="whitespace-nowrap px-3 py-2"><p className="font-medium text-healthcare-text-primary dark:text-healthcare-text-primary-dark">{capability.label}</p><code className="text-xs text-healthcare-text-secondary">{capability.capability}</code></td>
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {aliases.map(({ alias, canonical }) => (
                <div key={alias} className="flex items-center gap-3 rounded-md border border-healthcare-border p-3 dark:border-healthcare-border-dark">
                  <UsersRound className="h-4 w-4 text-healthcare-text-secondary" aria-hidden="true" />
                  <code className="text-xs text-healthcare-text-primary dark:text-healthcare-text-primary-dark">{alias}</code>
                  <span className="text-healthcare-text-secondary">→</span>
              ...
              <div className="flex items-center gap-3 rounded-md border border-healthcare-success/40 bg-healthcare-success/5 p-3">
                ...
                <span className="text-xs text-healthcare-text-secondary">Inactive users and unknown roles always fail closed.</span>
              </div>
            </div>
          </section>
        </div>
