...
const statusClasses: Record<HealthStatus | 'degraded', string> = {
 ...
 unknown: 'border-healthcare-border bg-healthcare-surface-secondary text-healthcare-text-secondary dark:text-healthcare-text-secondary-dark dark:border-healthcare-border-dark dark:bg-healthcare-surface-hover-dark',
 disabled: 'border-healthcare-border bg-healthcare-surface-secondary text-healthcare-text-secondary dark:text-healthcare-text-secondary-dark dark:border-healthcare-border-dark dark:bg-healthcare-surface-hover-dark',
};
...
export default function SystemHealth({ snapshot: initialSnapshot, canRunDiagnostics }: SystemHealthProps) {
 ...
 {selected.required ? <span className="text-xs font-semibold uppercase text-healthcare-text-secondary dark:text-healthcare-text-secondary-dark">Required</span> : <span className="text-xs text-healthcare-text-secondary">Optional</span>}
 ...
 <div><dt className="text-xs font-medium text-healthcare-text-secondary dark:text-healthcare-text-secondary-dark">Owner</dt><dd className="mt-1 text-sm text-healthcare-text-primary dark:text-healthcare-text-primary-dark">{selected.owner}</dd></div>
 <div><dt className="text-xs font-medium text-healthcare-text-secondary dark:text-healthcare-text-secondary-dark">Observed</dt><dd className="mt-1 text-sm text-healthcare-text-primary dark:text-healthcare-text-primary-dark">{formatTime(selected.observedAt)}</dd></div>
 <div><dt className="text-xs font-medium text-healthcare-text-secondary dark:text-healthcare-text-secondary-dark">Origin / duration</dt><dd className="mt-1 text-sm text-healthcare-text-primary dark:text-healthcare-text-primary-dark">{selected.origin ?? 'None'} / {selected.durationMs === null ? 'Not observed' : `${selected.durationMs} ms`}</dd></div>
 <div><dt className="text-xs font-medium text-healthcare-text-secondary dark:text-healthcare-text-secondary-dark">Runbook</dt><dd className="mt-1 text-sm text-healthcare-text-primary dark:text-healthcare-text-primary-dark">{selected.runbookUrl ? <a className="text-healthcare-info" href={selected.runbookUrl}>{selected.runbookRef}</a> : selected.runbookRef}</dd></div>
 </dl>
 {selected.errorCode ? <p className="mt-3 font-mono text-xs text-healthcare-text-secondary dark:text-healthcare-text-secondary-dark">Error code: {selected.errorCode}</p> : null}
 ...
 className={`rounded-md px-2.5 py-1 text-xs font-medium capitalize ${filter === value ? 'bg-healthcare-primary text-white' : 'bg-healthcare-surface-secondary text-healthcare-text-secondary dark:text-healthcare-text-secondary-dark dark:bg-healthcare-surface-hover-dark'}`}
 >
 {value}
 </button>
 ))}
 ...
 <Icon className="h-4 w-4 text-healthcare-text-secondary dark:text-healthcare-text-secondary-dark" aria-hidden="true" />
