          ...
          <div className="grid gap-5 xl:grid-cols-[20rem_minmax(0,1fr)]">
            <aside>
              ...
              <div className="space-y-2">
                {campaigns.length === 0 ? <div className="rounded-md border border-dashed border-healthcare-border p-6 text-center text-sm text-healthcare-text-secondary dark:border-healthcare-border-dark dark:text-healthcare-text-secondary-dark">No campaign has been opened.</div> : campaigns.map((campaign) => (
            ...
            <main>
              {!selectedCampaign ? (
                <div className="rounded-md border border-dashed border-healthcare-border p-12 text-center dark:border-healthcare-border-dark"><ShieldCheck className="mx-auto h-7 w-7 text-healthcare-text-secondary" /><p className="mt-2 text-sm">Open a campaign to freeze the current privileged-access population.</p></div>
              ) : (
                <div className="space-y-4">
                  <section className="rounded-md border border-healthcare-border bg-healthcare-surface p-4 shadow-sm dark:border-healthcare-border-dark dark:bg-healthcare-surface-dark">
                    ...
                    <dl className="mt-3 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4">
                      <div><dt className="text-xs text-healthcare-text-secondary">Progress</dt><dd className="font-medium">{selectedCampaign.decidedCount}/{selectedCampaign.itemCount}</dd></div>
                      <div><dt className="text-xs text-healthcare-text-secondary">Revoke decisions</dt><dd className="font-medium">{selectedCampaign.revokeCount}</dd></div>
                    ...
                    {completed ? (
                      ...
                      <div className="mt-3 space-y-3">
                        ...
                        {!readyToComplete ? <p className="mt-1 text-xs text-healthcare-text-secondary">All items must be decided and every revocation remediated before completion.</p> : null}
                        <div className="max-w-2xl border-t border-healthcare-border pt-3 dark:border-healthcare-border-dark">
                          ...
                          <p className="mt-1 text-xs text-healthcare-text-secondary">Cancellation is step-up protected, audited, and immutable. It releases the quarter so a replacement campaign can be opened.</p>
                        </div>
                      </div>
                    ) : selectedCampaign.status === 'cancelled' ? <p className="mt-3 text-sm text-healthcare-text-secondary">Cancelled {formatTime(selectedCampaign.cancelledAt ?? null)} — {selectedCampaign.cancellationReason}</p> : null}
                    ...
                    <div className="space-y-3">
                      {selectedCampaign.items.map((item) => (
                        <article key={item.itemUuid} className="rounded-md border border-healthcare-border bg-healthcare-surface p-4 shadow-sm dark:border-healthcare-border-dark dark:bg-healthcare-surface-dark">
                          <div className="flex flex-wrap items-start justify-between gap-3">
                            <div><h3 className="font-medium">{item.subject.name} <span className="font-normal text-healthcare-text-secondary">({item.subject.username})</span></h3><p className="text-xs text-healthcare-text-secondary">Assigned reviewer: {item.reviewer.name} · snapshot role: {item.snapshot.scalar_role}</p></div>
                          ...
                          <div className="mt-3 grid gap-2 text-xs md:grid-cols-3">
                            <div><span className="text-healthcare-text-secondary">Roles</span><p>{item.snapshot.effective_roles.join(', ') || 'None'}</p></div>
