...
import type { LabBarrierReason, LabOldestItem } from '@/features/lab/schemas';

export default function BarrierAnnotationDrawer({ item, reasons, onSaved }: { item: LabOldestItem; reasons: LabBarrierReason[]; onSaved: () => void }) {
  const [open, setOpen] = useState(false);
  const [reasonCode, setReasonCode] = useState(reasons[0]?.reasonCode ?? '');
  const [description, setDescription] = useState('');
  const [owner, setOwner] = useState('Laboratory operations');
  const mutation = useMutation({
    mutationFn: () => axios.post('/api/lab/barriers', { orderUuid: item.orderUuid, reasonCode, description: description || null, owner: owner || null }),
  ...
  return <Dialog.Root open={open} onOpenChange={setOpen}>
    ...
    <Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-40 bg-black/40" /><Dialog.Content className="fixed inset-y-0 right-0 z-50 w-full max-w-md overflow-y-auto border-l border-healthcare-border bg-healthcare-surface p-6 shadow-xl dark:border-healthcare-border-dark dark:bg-healthcare-surface-dark">
      <div className="flex items-start justify-between gap-3"><div><Dialog.Title className="text-lg font-semibold text-healthcare-text-primary dark:text-healthcare-text-primary-dark">Annotate Laboratory barrier</Dialog.Title><Dialog.Description className="mt-1 text-sm text-healthcare-text-secondary dark:text-healthcare-text-secondary-dark">Link a governed reason to {item.label}. The action is audited.</Dialog.Description></div><Dialog.Close aria-label="Close barrier annotation" className="rounded-md p-1 focus:ring-2 focus:ring-healthcare-info"><X className="size-5" aria-hidden="true" /></Dialog.Close></div>
