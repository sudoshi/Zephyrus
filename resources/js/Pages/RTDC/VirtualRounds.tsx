...
import {
    ...
    patientDetailSchema,
    patientQuestionCandidatesResponseSchema,
    runsResponseSchema,
...
export default function VirtualRounds() {
    ...
    const effectiveTemplateUuid =
        templateUuid ?? templatesResponse?.data[0]?.template_uuid ?? null;

    const runsQuery = useRoundRuns(
        effectiveScopeKey ? `unit:${effectiveScopeKey}` : undefined,
    );
    const runs = useMemo(() => {
    ...
    const effectiveRunUuid =
        runUuid ?? openRun?.run_uuid ?? runs[0]?.run_uuid ?? null;
    ...
    const effectiveSelectedUuid =
        selectedPatientUuid ??
        board?.data.patients[0]?.round_patient_uuid ??
        null;
    ...
    const patientQuestionCandidatesQuery = useAvailablePatientQuestions(
        effectiveSelectedUuid,
    );
    const patientQuestions = useMemo(() => {
        if (patientQuestionCandidatesQuery.data === undefined) return [];
        const parsed = patientQuestionCandidatesResponseSchema.safeParse(
            patientQuestionCandidatesQuery.data,
        );
        return parsed.success ? parsed.data.data.patient_questions : [];
    }, [patientQuestionCandidatesQuery.data]);
    ...
    const handleMutationError = (error: unknown) => {
        if (error instanceof AxiosError && error.response?.status === 409) {
            const parsed = conflictResponseSchema.safeParse(
                error.response.data,
            );
            if (parsed.success) {
                setConflictMessage(
                    `${parsed.data.error.message} The view has been refreshed — please retry.`,
                );
                if (parsed.data.current && effectiveRunUuid) {
                    queryClient.setQueryData(
                        ["rounds", "board", effectiveRunUuid],
                        parsed.data.current,
                    );
                }
                return;
    ...
    const clearConflictThen = {
        onSuccess: () => setConflictMessage(null),
        onError: handleMutationError,
    };

    const flagDisabled =
        templatesQuery.error instanceof AxiosError &&
        templatesQuery.error.response?.status === 404;
            ...
            {flagDisabled ? (
                ...
                <>
                    <RoundsCommandBar
                        ...
                        selectedRun={
                            board?.data.run ??
                            runs.find((r) => r.run_uuid === effectiveRunUuid) ??
                            null
                        }
                        busy={busy}
                        onScopeChange={(key) => {
                        ...
                        onCreateRun={() => {
                            if (effectiveScopeKey && effectiveTemplateUuid) {
                                createRun.mutate(
                                    {
                                        template_uuid: effectiveTemplateUuid,
                                        scope_type: "unit",
                        ...
                        onLifecycle={(action) =>
                            lifecycle.mutate({ action }, clearConflictThen)
                        }
                    />
                        ...
                        <div className="flex flex-wrap items-center gap-3 text-xs text-healthcare-text-secondary dark:text-healthcare-text-secondary-dark">
                            <span>
                                ...
                                <span className="font-medium tabular-nums text-healthcare-text-primary dark:text-healthcare-text-primary-dark">
                                    {board.data.progress.rounded}/
                                    {board.data.progress.total}
                                </span>{" "}
                                rounded
                            </span>
                            <span>
                    ...
                    {boardQuery.isLoading && effectiveRunUuid && (
                        <ErrorCard message="Loading round…" />
                    )}
                            ...
                            <div>
                                {patientDetail ? (
                                    <RoundPatientWorkspace
                                        ...
                                        sections={
                                            templatesResponse?.meta.sections ??
                                            []
                                        }
                                        roles={
                                            templatesResponse?.meta.roles ?? {}
                                        }
                                        ...
                                        onTransition={(
                                            ...
                                            patientTransition.mutate(
                                                {
                                                    roundPatientUuid:
                                                        effectiveSelectedUuid as string,
                                                    action,
                                                    body,
                                                },
                                                clearConflictThen,
                                        ...
                                        onPromotePatientQuestion={(question) =>
                                            promotePatientQuestion
                                                .mutateAsync({
                                                    roundPatientUuid:
                                                        effectiveSelectedUuid as string,
                                                    input: {
                                                        ...
                                                        thread_version:
                                                            question.thread_version,
                                                        message_uuid:
                                                            question.message_uuid,
                                                    },
                                                })
                                                .then(() =>
                                                    setConflictMessage(null),
