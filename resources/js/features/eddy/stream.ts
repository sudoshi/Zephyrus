...
// A silent stream must surface as an error, never as a permanently blank
// reply: if no bytes arrive for this long the request is aborted and the
// composer is released for a retry.
 ...
 *
 * Every exit path calls exactly one of onComplete/onError — callers rely on
 * that to re-enable the composer.
 */
export async function streamEddyChat(input: SendChatInput, handlers: EddyStreamHandlers, signal?: AbortSignal): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      handlers.onError?.('Eddy is unavailable right now. Your message was not lost — try sending it again.');
      return;
    }

    if (!response.ok || !response.body) {
      handlers.onError?.('Eddy stream failed to start. Try again in a moment.');
      return;
    }
    ...
    try {
      ...
      handlers.onError?.(
        timedOut
          ? 'Eddy stopped responding and the request timed out. Try again.'
          : 'The connection to Eddy was interrupted. Try again.',
      );
      return;
    }
