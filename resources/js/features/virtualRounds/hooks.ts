...
export function useAvailablePatientQuestions(roundPatientUuid: string | null) {
    return useQuery<unknown>({
        queryKey: ["rounds", "patient-question-candidates", roundPatientUuid],
        queryFn: () =>
            fetchAvailablePatientQuestions(roundPatientUuid as string),
        enabled: roundPatientUuid !== null,
        staleTime: 10_000,
        retry: false,
    });
...
export function usePromotePatientQuestion() {
    const invalidate = useRoundsInvalidation();
    return useMutation({
        mutationFn: ({
            roundPatientUuid,
