...
export async function fetchRoundPatient(roundPatientUuid: string): Promise<unknown> {
...
export async function createRoundRun(input: {
  ...
  const res = await axios.post('/api/rounds/runs', input, { headers: idempotencyHeaders() });
  return res.data;
}

export type RunLifecycleAction = 'start' | 'pause' | 'resume' | 'complete' | 'cancel';
...
export async function runLifecycle(
  ...
  const res = await axios.post(`/api/rounds/runs/${runUuid}/${action}`, body, {
...
export async function reconcileCohort(
  ...
  const res = await axios.post(`/api/rounds/runs/${runUuid}/cohort/reconcile`, body, {
    headers: idempotencyHeaders(),
  });
  return res.data;
}
...
export type PatientTransitionAction = 'mark-ready' | 'complete' | 'reopen' | 'defer' | 'skip';
...
export async function transitionPatient(
  ...
  const res = await axios.post(`/api/rounds/patients/${roundPatientUuid}/${action}`, body, {
    headers: idempotencyHeaders(),
  });
  return res.data;
}
...
export async function composeContribution(
  ...
  const res = await axios.post(`/api/rounds/patients/${roundPatientUuid}/contributions`, body, {
    headers: idempotencyHeaders(),
  });
  return res.data;
}
...
export async function createQuestion(
  ...
  const res = await axios.post(`/api/rounds/patients/${roundPatientUuid}/questions`, body);
  return res.data;
}

export async function createTask(
  roundPatientUuid: string,
  body: { title: string; detail?: string; category?: string; owner_role?: string; due_at?: string },
): Promise<unknown> {
  const res = await axios.post(`/api/rounds/patients/${roundPatientUuid}/tasks`, body);
  return res.data;
}

export async function transitionTask(taskUuid: string, status: string): Promise<unknown> {
  const res = await axios.post(`/api/rounds/tasks/${taskUuid}/transition`, { status });
  return res.data;
}
