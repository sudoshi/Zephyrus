...
export const patientDetailSchema = z.object({
    data: z.object({
        ...
        bed: z.string().nullable(),
        unit_id: z.number().nullable(),
        requirements: requirementsSchema.extend({
            open_task_count: z.number(),
        }),
        rounded_at: z.string().nullable(),
        patient_label: z.string().nullable(),
...
// Staff-only bridge discovery projection. This is intentionally separate
// from the patient detail projection: it exists only behind the patient
// messaging pool and Virtual Rounds contributor checks.
export const patientQuestionCandidateSchema = z.object({
    thread_uuid: z.string(),
    thread_version: z.number(),
    message_uuid: z.string(),
    question_text: z.string(),
    sent_at: z.string().nullable(),
});

export const patientQuestionCandidatesResponseSchema = z.object({
    data: z.object({
        patient_questions: z.array(patientQuestionCandidateSchema),
    }),
    meta: roundsMetaSchema,
});
