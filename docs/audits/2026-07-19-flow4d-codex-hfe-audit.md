...
|---|-----|---------|-------------|
| F-1 | High | Role switcher and enforced flow lens are two independent systems (DashboardLayout renders RoleSwitcher above a scene bound to the server lens) | **RULED 2026-07-19 [SU]: server persona transition.** The switch performs a server/Inertia persona transition through `EnforceFlowLens` on lens-bound pages, and persona propagates to every lensed request/SSE. → `feature/flow4d-hfe-rulings` |
| F-2 | High | Rounds stops disclose bed-level context under aggregate lenses (`rounds: true` for every dots policy; redactor spares `bed`) | **RULED 2026-07-19 [SU]: centroid + server redaction.** Under `patient_dots = none` the projection anchors stops at unit centroids and strips bed/facility-space/board deep link server-side; sentinel test rejects bed/location identifiers for aggregate lenses. → `feature/flow4d-hfe-rulings` |
| F-3 | High | Ordinary 18h stays are classified `delayed` (unverified/inferred) and can cascade up to six coral objects per location | **Split → RULED 2026-07-19 [SU]: inferred capped at amber.** Census-script half **fixed now** (urgency census counts rendered coral objects — PR #43). Classification: unverified duration-only risk maxes at `watch`; coral requires verification; per-service validated targets are the phase-2 refinement. → `feature/flow4d-hfe-rulings` |
