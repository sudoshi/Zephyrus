...
| F-9 | Medium | Hard-coded dark island, cyan focus ring in the toolbar, no reduced-motion branch | **Split.** Focus ring cyan→gold **fixed now** (PR #43 — canon alignment inside the same file that already uses gold everywhere else). Scoped light theme + `prefers-reduced-motion` are **follow-up**; dark-only was a deliberate wall-display choice that DESIGN.md's dual-theme doctrine contradicts — flagged for ruling. |
