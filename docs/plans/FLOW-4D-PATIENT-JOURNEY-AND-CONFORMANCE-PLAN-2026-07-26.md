...
Flag: `FLOW4D_CONFORMANCE_ENABLED` (config `services.flow4d.conformance`, default **false**; Inertia-shared like `arena.ai_enabled`).
- [ ] C1 Adherence panel in journey drawer per §7.2 (headline elements-met, two-lane expected/observed, pattern-language deviations, provenance + batch-time line, exception-note draft via Eddy plane [`EddyActionService::propose(approve:false)` — never hand-rolled], copilot explain when AI enabled)
- [ ] C2 Deviation pattern translation layer `features/patientFlowNavigator/adherence.ts` (code → {pattern, clinician copy, evidence times}); unit-tested against all current deviation codes of the 3 pathways
- [ ] C3 Scene: deviation glyph (new sceneVocabulary entry — shape TBD in-phase, distinct from triangle/diamond/torus; amber cap enforced in vocabulary), "Pathway" layer toggle (default off), census scope `Pathway deviations` w/ chip + metric relabel + explicit Focus; legend + hover wording; `cvdPalette.test.ts` extended to the new pair set (AT-3)
- [ ] C4 Freshness honesty: every adherence element carries "as of {batch}" and the panel states batch cadence; no element ever labeled "live" (CF-4) — streaming upgrade is deliberately out of scope here
- [ ] C5 Gates composed: flag ∧ `ARENA_ENABLED` ∧ full-dots lens; aggregate personas see nothing; PHPUnit route-gate tests all 8 combinations
- [ ] `scripts/urgency-census-flow4d.mjs`: count deviation glyphs into the coral/amber shares; soak hook exposes glyph counts
- [ ] Usability protocol draft task T9 (SAGAT probe: "is this patient on or off their sepsis bundle, and how do you know?") staged for H3
**Acceptance:** with the flag on (dev), a deviant seeded sepsis case shows: amber bracket glyph in-scene → drawer panel names the late/missing steps in clinician language with provenance and batch time → census scope isolates all deviant-case locations → Eddy can draft an exception note that lands PENDING. With the flag off: byte-identical navigator.
