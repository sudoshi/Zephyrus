...
### Phase C — Operational adherence surface (~3 d) `feature/flow4d-adherence` [C, enable = SU] — **SHIPPED (PR #98, squash `a979e786`) + ENABLED ON PROD ([SU] OQ-1 ruling 2026-07-27; amber cap stands)**
...
### Phase D — Governed DRG catalog bridge (~4–5 d) `feature/flow4d-pathway-bridge` [C build; SU/clinical to ever serve] — **SHIPPED DARK (PR #115, branch `64ca48cf`); NOT deployed-visible beyond the demo overlay; serving stays off pending [SU]/clinical (G-9)**
...
- [x] D1 `app/Services/CarePathways/ReferenceModelCompiler.php` (placed with the care-pathways service cluster, not `app/Domain/`): version → the *declarative* portion of the sidecar spec (ordered activities + timing targets + data-driven deviation labels) + a content-address digest; pure `compile()` unit-tested (determinism, digest pin, ordering, both `expected_range` shapes) + a thin `compileVersion()` DB adapter that returns **null** for a version with no persisted executable layer — which, for the inactive release (`milestone_definitions` empty), is every version. **Scope note:** the compiler does NOT emit the sidecar's `evaluate` callable (imperative timing logic, not data-derivable); no sidecar registration endpoint added (PATHWAYS stays hardcoded/stateless — the honest fit given the callable).
- [x] D2 Activity mapping (`config/care-pathways-ocel-map.php`, near-empty by design) + `care-pathways:ocel-coverage` command reporting unmapped milestones + targets outside the OCEL vocabulary — the honest gap list clinical review will need (reports the empty layer honestly on the current catalog).
- [x] D3 (spike) CPG-on-FHIR PlanDefinition + CQL vs bespoke spec → decision memo `docs/plans/FLOW-4D-D3-CPG-ON-FHIR-DECISION-MEMO-2026-07-27.md`: adopt PlanDefinition as canonical *representation* (projection follow-up, gated with serving), defer CQL *execution* until there's an executable requirement, ship the bespoke declarative compiler now. No dependency added.
- [x] D4 `PathwayInstanceReadService` over `patient_experience.pathway_instances` + the `current_*_statuses` views (append-only reads); `available()` = `assignment_enabled` ∧ `Schema::hasTable` (inert on dev — tables absent + flag off); resolves the active grant via `encounter_access_grants.source_encounter_id`; flag check at service AND (belt-and-suspenders) the orchestrator/component.
- [x] D5 Demo overlay: `FlowPathwayDemoService` synthesizes curated HF milestones → the **same** pure compiler → LOS-driven progress attached to one designated demo patient (earliest-admit active inpatient) in the journey drawer; no DB writes, `clinical_use=false`, "Demo — not clinical guidance" notice; `PathwayProgressPanel` (status by glyph+word, F-9 scoped palette); new `flow4d.pathway_progress` serving flag composes with `assignment_enabled` (Inertia `flow4d.pathway_progress_enabled`); demo rides `care_pathways.demo`.
- [x] Tests: compiler determinism + digest pin (content-addressed); flag-off inertness (journey response omits `pathway_progress` — byte-identical); append-only discipline untouched (reads never write). 25 PHP + 6 JS, all green; tsc + vite build + check-ui-canon + Pint clean.
**Acceptance:** a governed pathway version compiles to a conformance-checkable reference model with a coverage report; demo overlay demonstrates the full loop on synthetic data; nothing clinical serves anywhere with flags off — proven by tests, asserted with G-9's convention (no inferring clinical approval). **Met.** Deferred-with-serving: the `PlanDefinition` projection (D3) and any real per-patient serving from the DRG catalog ([SU]/clinical, G-9).

### Phase E — Wayfinding & 2D structure (~4 d) `feature/flow4d-wayfinding` [C] — **SHIPPED + DEPLOYED PROD (PR #107, squash `5f01dd81`; deployed 2026-07-27 at `17dd51c6`)**
- [x] E1 Minimap/floor-stack widget (camera footprint, selection dot, delay/deviation pips; click = fit-to-floor); CVD + never-color-alone treatment; suppress in wall/kiosk param — `minimapProjection.ts` (pure round-trip) + `NavigatorMinimap` SVG plate, polls camera/selection from the scene so the ~7 Hz camera stream never re-renders the orchestrator; `?wall=1` suppressed
- [x] E2 Canonical views control (Top/House/Floor/Bed) + van Wijk-arc verification/upgrade of `focusOn`/`flyTo` transitions; SDF-text unit billboards w/ LOD + distance culling — `cameraFlight.ts` (van Wijk & Nuij ρ*=1.42, 8 property tests); `troika-three-text` billboards (local ambient .d.ts)
- [x] E3 Top-down orthographic mode (`O`) — all layers; disks/labels legibility pass; Data-Navigator-style structure traversal (floor→unit→bed arrow-key graph) layered on the existing action-list seam — perspective/ortho camera swap under OrbitControls; `structureTree.ts` + `NavigatorStructureNav` WAI-ARIA tree (roving arrow-key graph, shared selectEntity seam)
- [x] E4 Flatten + small multiples: "Trail heat (last 6 h)" scene mode (GSTC flatten) and a 6-slice hourly small-multiples export card (analysis path replacing forced replay) — `trailHeat.ts` + `NavigatorSmallMultiples` (SVG export + on-floor flatten via transient scene layer)
- [x] E5 URL view state + "Copy view link" (camera/floor/layers/time/selection/censusScope; extends parseHandoff; saved views gain "copy as link") — `viewState.ts`, seeded 200-view round-trip; `sel=` aggregate-only, patient travels only as opaque ptok. TN-6 chronobar presets (48h/24h/6h/Shift) shipped alongside
- [x] E6 Task modes Monitor/Investigate/Rounds (progressive disclosure; `NavigatorToolbar` decomposition — the renderToolbar test fixture must gain the mode prop; WN-6)
- [x] Tests: mode-gated control visibility, URL round-trip property test, minimap sync, ortho toggle (unit); LOD label culling threshold — verified in-scene, browser-covered by F5
- *Deferred (plan-consistent, B4): outward "Locate in 4D" buttons on ED/bed boards — those rows carry no opaque ptok client-side; needs backend ptok minting + re-id review, scope separately.*
**Acceptance:** an operator can always answer "where am I / what am I looking at / how do I get back" in one glance + one click; every monitoring answer has a non-perspectival path; any exact view is shareable as a URL. **MET** — 221 vitest green (viewState/cameraFlight/structureTree/minimapProjection + component suites), tsc + vite build clean.

### Phase F — Resilience & scale (~3–4 d) `feature/flow4d-resilience` [C] — **F1/F4/F5/F2b SHIPPED + DEPLOYED PROD (PR #108, squash `17dd51c6`, deployed 2026-07-27); F2a + F3 DEFERRED**
- [x] F1 Context-loss recovery (absorbs Codex F-11): `webglcontextlost` preventDefault + halt + visible degraded card ("Graphics paused"); `webglcontextrestored` → full rebuild from state (scene state lives outside GL); deterministic test via `WEBGL_lose_context` in the browser test (F5); soak hook counts losses/restores (`contextEvents()`)
- [~] F2 **F2(b) SHIPPED** — aggregate-trails guard: >50 concurrent trails → unit→unit flow arrows w/ counts, individual trail on selection (§6.2); pure `flowAggregation.ts` tally (5 tests). **F2(a) InstancedMesh migration DEFERRED** — premature optimization at demo scale (~40 pts); more importantly F5's selection-survival can't run in CI without an E2E flow seed, so a large token-rendering refactor would be unguarded. Land with the E2E seed so F5 verifies instanced selection.
- [ ] F3 Delta updates (pre-sized instance buffers + `?since=` web deltas) — **DEFERRED with F2(a):** the instance-buffer path depends on instancing; the `?since=` web-delta is a coupled data-path change best landed together
- [x] F4 Frame budget: per-subsystem (frame/render/labels/controls) p95 timing via ring buffers (`performance.now` deltas, no measure-entry churn); exposed on the soak hook (`frameBudget()`); RAIL budget table to be filled from the H4 soak run (G4)
- [x] F5 Rendered-scene Playwright test (absorbs Codex F-12): boot scene → force layer rebuild → renderer alive; context-loss self-heal + counters; frame-budget accumulates; `tests/e2e/patient-flow-navigator.spec.ts` in the browser CI job. Graceful skip when CI lacks WebGL/flow data (E2eTestSeeder carries none — model boots into an empty data layer); auto-strengthens when an E2E flow seed lands
**Acceptance:** wall survives GPU reset with self-recovery **(MET, F1+F5)**; browser CI covers the long-session failure modes end-to-end **(MET for boot/context-loss/frame-budget/layer-rebuild)**. Draw-calls-flat-vs-entity-count (instancing soak counters) **carries to the F2(a) follow-up**. 221 vitest green; tsc + vite build clean; playwright --list resolves the 4 rendered-scene tests.
...
|---|---|---|
| OQ-1 | **Adherence enablement + coral eligibility.** Enable `FLOW4D_CONFORMANCE_ENABLED` on prod after Phase C? And: may any *observed* hard breach (e.g., `antibiotic_late`) ever escalate the glyph to coral, or is pathway state permanently amber-capped in-scene? | **RULED [SU] 2026-07-27: ENABLED on prod** (flag live, config cached, fpm restarted; verified composed=true and 31 deviant sepsis patients joining the live census from the 18:00Z batch). **Amber-only cap stands in v1** (structurally enforced in sceneVocabulary + pinned by cvdPalette.test.ts); coral eligibility revisits only after H3 evidence, per the recommendation. |
