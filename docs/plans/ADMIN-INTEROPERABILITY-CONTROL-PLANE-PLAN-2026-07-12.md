...
**Implementation status (2026-07-16):** Phase 0 production safety, the Admin cockpit, enterprise/source scope, governed source lifecycle, source onboarding/readiness/scheduled activation, credential/network authority, operational observability, vendor-neutral FHIR polling, governed resource profiles, immutable bounded FHIR/SMART discovery evidence, and standards-correct FHIR R4 type-search/type-history polling are code-complete on `feat/admin-production-safety`. Checked items below mean code-complete with automated evidence on that branch; they do not waive production smoke, institutional provider/bootstrap configuration, raw-PHI migration/cutover, partner conformance, or go-live gates.

**Audit date:** 2026-07-12

**Product authority:** `docs/product/ZEPHYRUS-2.0-BETA-PRD.md` remains the application-wide product authority. This document is the execution authority for Administration, enterprise setup, integration governance, and healthcare transactional interoperability.

**Supersedes:** The remaining Release C-H sequence in `docs/archive/PARTHENON-ADMIN-PORT-PLAN-2026-07-10.md` and the administration/control-plane backlog in `docs/superpowers/plans/2026-06-25-real-time-healthcare-data-acquisition.md`. Those documents remain useful background and standards research.
...
- FHIR R4 SMART Backend Services discovery, a vendor-neutral polling core, vendor conformance policies, and a governed dynamic resource-profile registry.
...
- The 2026-07-16 search/history closeout reconciled `origin/main`, then proved the standards/runtime tranche with **117 focused integration tests / 1,445 assertions**. The authoritative full backend run passed **1,522 tests / 95,377 assertions** with one intentional skip before the later browser-fixture-only changes; the changed cross-domain seeder then passed its focused **1 test / 21 assertions**, and the changed Radiology model contract passed **5 tests / 277 assertions**. Final frontend proof passed **128 Vitest files / 530 tests**, TypeScript, local Iconify bundle drift validation, Composer validation, shell syntax, Pint over all **28 changed PHP files**, `git diff --check`, and two production Vite builds. The complete fresh-migration authenticated browser lane passed **92 tests** with nine deliberate state-capture skips and zero failures across 101 cases; evidence is retained at `artifacts/release-evidence/browser/20260716T173214Z-browser-playwright.json`, and its disposable database and per-run encrypted clinical-payload resources were removed. Browser validation drove strict contract fixes for Lab forecasting and Radiology Brier-score provenance, a deterministic RTDC/Perioperative/Cockpit/Lab/Radiology/Pharmacy fixture, a CSP-compatible local bundle of 191 Iconify assets, and same-host/default-off realtime initialization. Production was not changed.
...
| FHIR | Discovery, R4 validation, vendor-neutral SMART client, vendor policy seam, governed dynamic resource profiles | Complete capability/search/profile capture, US Core profiles, resource mappers, Bulk Data, subscriptions, reconciliation, write operations, conformance suite |
...
The active-scope contract is explicit and fail closed: no role, including a global administrator, receives an automatic organization, facility, or source. The session payload is bound to the authenticated user and re-resolved on every request against the current active facility hierarchy, source membership, and effective-dated access grant. Revocation, account switching, stale hierarchy, malformed identifiers, and source mismatch clear or reject the boundary. Every Admin integration mutation declares `facility`, `source`, and/or `governed_change` middleware, and a route-inventory regression fails if that exact matrix changes silently. The React selector filters each level to a coherent hierarchy; source, endpoint, credential, FHIR, health, replay, governed-decision, and writeback controls cannot mutate a different source. Read summaries remain capability-wide and say so explicitly.

A real-browser automated WCAG 2.2 AA gate now ships: `tests/e2e/admin-accessibility.spec.ts` (via `@axe-core/playwright`) scans every admin surface — dashboard (unscoped + scoped), system health, auth providers, roles/capabilities, access reviews, data protection, users index + edit, cockpit thresholds, AI providers, and the enterprise/deployment console — with the wcag2a/2aa/21a/21aa/22aa rule sets and passes. Fixing it to green removed real defects: Inertia's invalid `role="bar"`/`role="spinner"` progress indicator (replaced by the decorative, `aria-hidden` `NavigationProgress` component, `progress:false`), missing `dark:` color-token pairs that rendered near-invisible secondary text on dark surfaces, a phantom `healthcare-surface-secondary-dark` token that broke badge contrast app-wide, non-keyboard-focusable scrollable regions, an invalid `<dd>` role that broke definition-list structure, and coral-on-coral-tint status text (moved to a paired icon + border + high-contrast label, honoring Status-Never-By-Color-Alone). The `[ ]` remains open for two residuals: (1) two systemic core-palette contrast items — white text on the `healthcare-info`/`healthcare-primary` interactive-blue fills in dark mode (3.7–4.1:1) and `healthcare-critical` coral text on the slate surface (4.25:1) — are held in an explicit, documented `KNOWN_BASELINE` in the spec because closing them requires a DESIGN.md palette decision that repaints interactive blue and status coral app-wide, not a local fix; and (2) a formal manual screen-reader / keyboard-task-completion audit, which no automated scan can substitute for.
...
- [x] Import and review the production organization, market, facility, service-line, location/space, capability, and transfer registry before source onboarding.
...
- [x] Add effective dating, source-of-truth, external identifiers, ownership, and change history to enterprise entities.
- [x] Add UI import preview, conflict review, commit, and readiness scoring.
- [x] Block source activation when its canonical organization/facility is missing, mismatched, or inactive.
- [x] Block source activation when required locations/service lines are missing or unresolved after the onboarding contract declares them.

**Branch implementation state:** `EnterpriseRegistryImportService` + `EnterpriseRegistryNormalizer` accept a registry payload (organizations, markets, facilities, service lines, locations/spaces) and compute a preview that classifies every record create/update/no_change/conflict/blocked with a readiness score; the governed commit (`manageEnterpriseSetup` + step-up + author≠approver via `GovernedChangeService`, action `ApplyEnterpriseRegistryImport`, exact-hash execution) applies the plan atomically as additive upserts with no destructive delete, binding operator conflict resolutions into the hash. Migration 001500 adds effective dating (`valid_from`/`valid_until`), `source_of_truth`, namespaced `external_identifiers`, owner/steward to `hosp_org.organizations|markets|facilities`, `hosp_ref.service_lines`, and `hosp_space.facility_spaces`, plus the append-only `hosp_org.enterprise_change_history` ledger (mutation-rejecting trigger + clinical-content guard) and `integration.source_required_topology`. The Enterprise Setup "Import" tab renders create/update/conflict counts, per-entity detail, per-conflict adopt/skip resolution, readiness scoring, the governed commit/decide/apply flow, and the change-history view. `SourceReadinessService::evaluate()` now also blocks activation when a source's declared required service-lines/locations are missing or unresolved against the registry (only when declared via `PUT /admin/enterprise/sources/{source}/required-topology`), bound into the readiness input hash. The actual production data load and the eventual removal of the tenant/facility compatibility columns remain deployment-owned cutover steps. Evidence: full backend 1129 passed / 1 skip (17,319 assertions), enterprise import feature test 7, Vitest 44 page tests, tsc/Pint/check-ui-canon green; `deployment:seed-registry` still passes.
...
- [x] Separate lifecycle state, protocol health, data freshness, conformance status, contract status, and incident status.
...
- [x] Add explicit mTLS server-peer trust/pinning policy metadata where a partner contract requires controls beyond normal CA and server-name verification.
...
- [x] Deliver threshold alerts through the INT-OBS acknowledgement, suppression, escalation, and incident lifecycle rather than treating a console badge as an alert.
...
Explicit mTLS peer pinning is now metadata plus enforcement: `integration.source_peer_pin_policies` (migration 001400) records pin mode (`none`/`spki_sha256`/`cert_sha256`/`issuer_dn`), one or more pinned fingerprints, expected peer subject/SAN, a `required` flag, and an effective interval per route; `CertificateInspector::peerFingerprints` computes the presented cert-SHA256 and SPKI-SHA256, and `IntegrationConnectionGuard` consults the policy at connection time and fails closed on a required mismatch or missing peer certificate. Credential-rotation threshold crossings (90/60/30/14/7-day and expired) now deliver through the shared `OperationalAlertDispatcher` used by SLO breaches — PHI-free/secret-free payloads passed through `ClinicalContentGuard`, channels env-gated and inert by default, one page per threshold band via the append-only `credential_rotation_alert_states` dedupe ledger, every attempt recorded in `integration.operational_alert_deliveries`. The `integrations:dispatch-credential-rotation-alerts` command is registered and scheduled daily.
...
- [x] Add queue age/backpressure, retry budget, circuit breaker, rate-limit state, and vendor maintenance awareness.
- [x] Add OpenTelemetry-compatible metrics/traces with PHI-safe attributes and correlation from receipt to projection/outbound ACK.
- [x] Add alerts, acknowledgement, suppression/maintenance, escalation, incident link, and post-incident review.
- [x] Add source health to every downstream Cockpit/Hummingbird/Eddy signal contract.

**Branch implementation state:** `integration.health_observations` is an append-only per-source history ledger (UPDATE/DELETE rejected by trigger) recording window bounds, observation/freshness timestamps, status, stable error codes, duration, runtime state, and origin; `SourceObservabilityService::observe()` writes history and projects a monotonic current status, and `POST /api/admin/integrations/sources/{source}/observations` is capability- and scope-gated (`operateIntegrations` + `admin.scope:source`). `integration.source_slo_definitions` normalizes all seven SLO metrics one-to-one against the governing onboarding version, versioned via `previous_definition_id`, append-only, with backfill from existing onboarding versions. Runtime pressure is executable rather than inferred: SMART FHIR polling and FHIR protocol-health partner calls enforce the latest per-source/connector state before network access; 429 responses parse bounded delta-seconds or HTTP-date `Retry-After`; transient transport/5xx failures alone advance the circuit; an open circuit rejects without a partner call; and a source-row lock grants one expiring half-open probe lease. `integration.source_runtime_pressure_events` records append-only rate/circuit transitions, while `integration.source_runtime_execution_events` records database-authority-checked, idempotent `queued`, `attempt_started`, `retry_scheduled`, pressure rejection, success, and terminal-failure evidence. Retry-budget consumption is measured from immutable attempt starts, not mutable run status; attempts without ledger evidence remain unknown. Focused feature coverage proves early re-entry blocking, no-third-call circuit rejection, half-open recovery, authority mismatch rejection, append-only enforcement, and the database clinical-content tripwire. `integration.slo_breaches`/`slo_breach_events` record opened/continued/suppressed/resumed/acknowledged/recovered/escalated/incident_linked/reviewed transitions; maintenance windows suppress notification without erasing the breach; and operator `acknowledge`/`escalate`/`link-incident`/`review` actions (`operateIntegrations` + `admin.scope:source`, reason-captured, audited, incident stored as SHA-256 only, review carrying bounded enum codes) close the workflow. Alert delivery flows through the shared `OperationalAlertDispatcher`/`OperationalAlertChannel` abstraction that generalizes the existing Teams/push channels (env-gated, inert by default, every attempt including no-ops recorded in append-only `integration.operational_alert_deliveries`, all text passed through `ClinicalContentGuard`), firing on the non-suppressed breach-open transition and on escalation. `SourceHealthDigestService` embeds a PHI-free per-source status/freshness/staleness/mode block additively into the Cockpit snapshot (frozen Zod extended with an optional `sourceHealth`), Eddy context, and thereby the Hummingbird BFF, so stale/degraded sources visibly degrade downstream. The default-off OTLP wire now uses the official OpenTelemetry PHP SDK and exporter over HTTP/protobuf. Exact-host allowlisting permits HTTPS or loopback HTTP only; optional authentication headers resolve through the governed secret-reference provider and reject CR/LF injection; SDK exception logging is disabled; and resource/record attributes pass `SpanAttributes` plus `ClinicalContentGuard`. Guarded spans cover SMART FHIR polling, canonical persistence, RTDC projection, and synchronous HL7 receipt-to-projection, correlated by assigned request UUID and canonical event UUID without accepting inbound trace state. Unit coverage decodes actual protobuf and feature coverage proves correlation with clinical fixture content absent. `docs/operations/INTEGRATIONS-OBSERVABILITY-OTLP-RUNBOOK.md` defines Collector-agent topology, bounded failure behavior, activation, validation, and rollback.

### Phase 3 - Complete the Core Clinical Transaction Engines

#### FHIR-CORE - Generalize the FHIR implementation

- [x] Refactor `EpicSmartFhirClient` into a vendor-neutral SMART Backend FHIR client plus an Epic conformance profile; preserve Epic-specific behavior outside the protocol core.
- [x] Capture full CapabilityStatement details: FHIR version, formats, profiles, interactions, operations, compartments, search parameters, includes/revincludes, batch/transaction, subscriptions, and Bulk Data endpoints.
- [x] Capture `.well-known/smart-configuration`, authorization/token endpoints, capabilities, algorithms, scopes, and issuer validation.
- [x] Support bounded type-search paging with inclusive `_lastUpdated`, `_count`, strict parameter handling, 429/Retry-After, bounded retries, safe OperationOutcome handling, fail-closed bundle validation, and same-origin next links.
- [x] Support governed type-history paging with inclusive `_since`, response ETag version capture, create/update versions, deletion tombstones, immutable provenance, separate watermarks, and the same page/resource/origin bounds.
- [ ] Add conditional read/vread workflows using `If-None-Match` and `If-Modified-Since` where a mapper requires direct resource reads; do not apply read-only conditional semantics to search or history requests.
- [x] Replace the hard-coded Encounter/Location poll allowlist with a governed resource profile registry.
...
**Branch implementation state:** `SmartBackendFhirClient` now owns vendor-neutral SMART Backend Services token exchange, R4 search, paging, origin validation, versioned persistence, watermarks, and PHI-safe telemetry. `EpicSmartFhirClient` and `PollEpicFhirResource` remain compatibility facades for existing callers and serialized work; new queue work uses `PollFhirResource`. Optional vendor assertions are isolated in `FhirVendorConformanceService` and configured as source-vendor policy rather than protocol behavior. `integration.fhir_resource_profiles` is the guarded current authority for configured resource type, canonical profile identity, poll intent, cadence, page/resource limits, actor, correlation, and PHI-free change reason; exact-version updates automatically append immutable `fhir_resource_profile_events`, hard delete is rejected, configuration authority must match the source, clinical-content canaries are rejected, and `enabled` requires both a discovered CapabilityStatement resource and a SMART `system/{resource}.read`/read-capable v2 scope. Scoped `manageIntegrations` APIs and React controls configure, revise, disable, and retire profiles without direct database editing. Direct changes fail closed for production, PHI-approved, live, scheduled, or otherwise protected sources, which must use governed reconfiguration, independent approval, revalidation, and activation. Discovery reconciles configured profiles to `enabled` or `suspended`; the control-plane API renders them dynamically; manual and scheduled dispatch use the same registry and queue generic jobs. Empty queue/runtime state is preserved as a JSON object, so a freshly migrated source without a client connection remains a valid strict frontend contract rather than collapsing the console into an unavailable state. Focused proof passes **10 backend tests / 168 assertions** plus **12 FHIR/API frontend tests**, including non-Epic discovery and poll, dynamic Observation configuration/dispatch/retirement, scheduler coverage, capability/scope loss, append-only history, no-delete, strict authorization, protected-source rejection, and clinical-content rejection. The prior authoritative gates passed **1,136 backend tests / 17,434 assertions** with one intentional skip, **105 Vitest files / 448 tests**, TypeScript, Laravel Pint across **1,255 PHP files**, `git diff --check`, Composer validation, and the production Vite build. The fresh-migration authenticated Chromium lane passed **33 tests** with three deliberate feature skips, including scoped FHIR resource-governance rendering and fail-closed controls; evidence is retained at `artifacts/release-evidence/browser/20260715T235720Z-browser-playwright.json`, and its disposable database was removed.

**Capability/SMART discovery tranche (2026-07-15):** every successful health check now normalizes the full bounded FHIR R4 CapabilityStatement server surface and SMART configuration into `integration.fhir_conformance_observations` plus resource-detail children. The append-only, source-bound observation chain records document hashes; FHIR version/status/date/kind; software and implementation origin; formats, patch formats, implementation guides, compartments, security services, system interactions/operations; per-resource base/supported profiles, interactions, versioning, conditionals, search parameters, includes/revincludes, and operations; SMART issuer/JWKS/authorization/token/registration/management/introspection endpoints, grants, client authentication methods/algorithms, scopes, capabilities, PKCE methods, and associated endpoints; and derived batch, transaction, history, search, subscription, and Bulk Data support. Parsing fails closed on non-R4/non-server statements, missing required metadata, unsafe endpoints, unsupported backend-service grants/authentication, interactive authorization without S256, endpoint/credential token drift, malformed arrays, or the explicit 500-resource, 500-search-parameter-per-resource, and 100-operation-per-resource bounds. PostgreSQL rejects history rewrites, pointer rewind/clear, count-inconsistent finalization, mismatched source authority, late child insertion, and recognizable clinical content. Resources advertising either `search-type` or `history-type` become capability inputs, while a governed profile is enabled only for its exact selected interaction and an authorized SMART system-read scope. A source-scoped read API and strict React contract render PHI-free counts, hashes, endpoint origins, flags, warnings, and up to 100 resource rows; they render unobserved/loading/error states truthfully and never expose full discovered URLs. Browser validation also drove an Inertia-safe tab-history fix so clearing Admin scope preserves `tab=fhir`. The final authoritative gates pass **1,139 backend tests / 17,476 assertions** with one intentional skip, **105 Vitest files / 451 tests**, TypeScript, Laravel Pint across **1,257 PHP files**, `git diff --check`, Composer validation, and the production Vite build. The complete fresh-migration authenticated Chromium lane passes **33 tests** with three deliberate feature skips; evidence is retained at `artifacts/release-evidence/browser/20260716T012746Z-browser-playwright.json`, and its disposable database was removed. Discovery evidence identifies advertised support; it does not claim those workflows are implemented or partner-certified. Production was not changed.

**Search/history runtime tranche (2026-07-16):** the resource-profile registry now governs `search` versus `history` per resource, defaults existing profiles to search, records every interaction change in the existing immutable profile-event chain, and fails closed unless the latest CapabilityStatement advertises the exact `search-type` or `history-type` interaction. The Admin FHIR panel exposes that choice and labels the resulting behavior. Search sends `Prefer: handling=strict`, re-queries its last boundary with `_lastUpdated=ge...` so timestamp precision cannot create gaps, accepts only `searchset`, distinguishes match/include/outcome entries, persists only valid same-type matches, and treats fatal/error OperationOutcome issues or malformed match entries as a failed poll with no watermark advance. Informational/warning outcomes are counted but their diagnostics are never stored. Type history uses `[type]/_history`, an inclusive `_since` cursor, and a separate `history_since` watermark; PUT/POST entries reuse encrypted immutable resource persistence, while DELETE entries create payload-free version tombstones from the bounded target, ETag, and `response.lastModified` with raw receipt and provenance records. Both paths retain page/resource limits, same-origin next-link enforcement, runtime pressure/circuit behavior, stable PHI-safe failures, and duplicate-safe idempotency. Focused proof is green at **117 integration tests / 1,445 assertions**, including inclusive boundary replay, strict handling, warning/error OperationOutcome behavior, encrypted history versions, tombstones, cursor reuse, and capability withdrawal; the final complete frontend suite is green at **128 files / 530 tests**, and the complete fresh-migration browser gate is green at **92 passed / 9 intentionally skipped / 0 failed**. Remaining FHIR work is direct conditional reads where required, dependency-ordered resource mappers/crosswalks and validator packages, actual Bulk Data and subscription execution, and source reconciliation. Production was not changed.
...
1. Generalize the FHIR client and pin the selected EHR sandbox profile. **Protocol core complete; a named institutional profile and signed partner evidence remain.**
...
**Current dependency order (2026-07-16):** first select the named institution/vendor/version/IG and import only non-confidential profile evidence; then implement the Patient → Organization/Location → Practitioner/Role → Encounter mapper/crosswalk spine with exact validator packages and reconciliation; then complete the matching HL7 ADT interface-engine ACK/correction profile against the same source. Bulk Data, subscriptions, orders/results, imaging, pharmacy, payer, document/HIE, public-health, device, and outbound transaction packs remain later source-scoped tranches and must not be represented as production-supported before their individual G0-G4 evidence is signed.
