...
| **Companion doc** | `EDDY-ABBY-TEARDOWN-EVIDENCE.md` — the raw, `file:line`-cited teardown maps this plan is built on |
...
Abby is a four-layer system (full `file:line` evidence in the companion `EDDY-ABBY-TEARDOWN-EVIDENCE.md`):
...
> **Reading note.** Each Part retains its author's internal section numbering (e.g. Part A's "§4.2", Part F's "§A–F") and intra-Part cross-references. These are *Part-local*. Where Parts disagree on incidental naming, §4 above is authoritative. The companion `EDDY-ABBY-TEARDOWN-EVIDENCE.md` holds the nine raw teardown maps with `file:line` citations that every Part is built on.
