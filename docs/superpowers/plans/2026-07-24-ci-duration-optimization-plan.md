...
- **Required-checks contract:** 17 required contexts exist by *name*. Any job rename/merge/removal must update branch protection in the same change window or PRs become unmergeable (this kills the "delete the DAST job" variant — DAST **is** required). *(Post-#91 update: 13 required contexts — `Backend tests (feature-4)`..`(feature-7)` were removed with the 8→4 shard consolidation, strict mode preserved. Do not re-add them. Post-tranche updates 2026-07-27: **15** required contexts — the iOS build-side tranche added `Hummingbird patient (iOS release)` (13→14, PR #99) and the Browser-chain decouple added `Frontend build (Vite)` (14→15, PR #103); both are security/build gates that would otherwise stop gating merges when a build/verify step moved to a parallel sibling job.)*
...
- [x] Pint `--parallel` (`ci.yml:117`): quality job 101 s → ~35 s. **DEFERRED:** needs Pint ≥ 1.22; the 1.20→1.29.3 bump restyles ~150 files — dedicated bump+reformat PR. *Shipped 2026-07-26 as that dedicated PR (#89): `laravel/pint: ^1.29` + the full restyle; Pint gate ~40 s → ~0.5 s. Gotcha: Pint 1.29's fixer chain is not single-pass idempotent — after a bulk reformat, rerun `--test` serial (local `--test --parallel` missed one file that CI caught).*
...
- [x] In order: (a) test whether `macos-15`'s Xcode selection now covers the needed iOS-26 SDK — if yes, leave the scarce pool entirely; (b) evaluate larger-runner pools; (c) failing both, move XCUITest journeys to main-push/nightly lanes only (permissible — staff iOS is not a required PR check — but record the coverage trade-off explicitly). *Shipped 2026-07-25 via option (a) (PR #71): `macos-15` ships Xcode 26.0–26.3 side-by-side with its 16.4 default — pinned via `DEVELOPER_DIR`; queue delay up to 35.4 m → 4 s for +3.7 m work time. The old ci.yml comment claiming macos-15 "cannot parse" the iOS-26 SDK described only the image default.*
- [x] Either way: emit queue-delay (job `started_at` − run `created_at`) into release evidence so the tail is *measured* continuously. *Dispositioned at closeout (2026-07-27): not implemented — the runner move removed the tail it was meant to measure (4 s observed vs up to 35.4 m); standing evidence not warranted while staff iOS sits on the plentiful pool. Revisit only if the job returns to a scarce runner pool.*
...
- [x] **D1. Profile `AncillaryDemoScenarioService::refresh` + `SnapshotBuilder::buildWithContext` + Lab services** with `EXPLAIN (ANALYZE, BUFFERS, WAL)` on the 66-order/328-milestone fixture in a disposable DB (§3.2.9 verbatim). Every shard layout is lower-bounded by the 625.8 s `AncillaryDemoScenarioTest`; a 2× refresh cut takes backend to ~6.5–7 m and is what ratifies the stage-2 ≤15 m budget. *Profiled 2026-07-25 (docs/audits/D1-ancillary-refresh-profile-2026-07-25.md): root cause = stale planner stats on freshly provisioned test DBs (nested-loop misplans, 15.2 s of the 25.6 s roll-forward); targeted post-refresh ANALYZE shipped — AncillaryDemoScenarioTest 201→123 s locally (1.64×). Remaining candidates (reconciliation-key index, batched status updates, provenance index) recorded for D2/D3.* *Follow-ups shipped 2026-07-25 (PR #78, deployed to prod): reconciliation lookup index rebuilt with a provable `IS NOT NULL` partial predicate (the 2026_07_11 predicate was unprovable from the `->>` clause — dead index left in place pending approval — **dropped 2026-07-27 after [SU] approval**, migration `2026_07_27_000100`; prod `pg_stat_user_indexes` at drop time: 0 scans on the dead index vs 2,303 on the successor), `provenance_records(canonical_event_id)` FK index, and `markProjected()` batching the 672 per-event projection flips into chunked whereIn updates. AncillaryDemoScenarioTest 92.9→82.0 s local on top of D1. Note: #78 is NOT plan-§D2 below — the audit's "for D2/D3" label collided with the plan's item names.*
- [x] **D2. Request-scoped laboratory aggregate snapshot** (one governed calculation shared by Cockpit provider, health service, drill) — §3.2.9's named remediation. *Shipped 2026-07-26 (PR #84, deployed to prod): `LabAggregateSnapshotFactory` (container `scoped()` — flushed per FPM request and per queue job) memoizes the {flow-board cockpitHealth, decision readinessSnapshot} pair eagerly together for the verified-empty reconciliation; `LabCockpitHealthService` consumes it; `cockpitHealthFrom()` derives the cockpit seam without re-running the queue pipeline; `SnapshotBuilder::refresh()` invalidates it so the authoritative snapshot never publishes a memo across a long-lived process's data changes. Scope is deliberately the named cockpit chain ONLY: memoizing `AncillaryReadinessService` is UNSAFE — the demo generators consume it mid-refresh (canonicalEvent → clinicalContext → DischargePrioritiesService → laboratoryAxes) while the cohort is being rewritten, proven by stack trace when the committed-scenario build memoized an empty mid-transaction cohort; a memo there would change generated demo data. All four governed constraints preserved (single-snapshot reconciliation, freshness semantics, privacy projection via pre-projection aggregates only, status-engine ownership); the user-filtered workspace `build()` paths stay unmemoized. Lab consumer classes 38.5→35.9 s local; 635-test sweep green.*
- [x] **D3. Paratest spike** (per-process isolation already exists: DB name keys on pid, `IsolatedTestDatabase.php:27`; array cache/session; pid-scoped disk). Only after D1 — it cannot break the single-class floor. Success enables 8 → 4 shard consolidation. *Spiked 2026-07-26 in an isolated clone — **FEASIBLE**, consolidation PR deferred until §D2 lands + fresh manifest evidence. Findings: paratest 7.8 installs under PHP 8.5 (Pest can't); pid-keyed DB provisioning composes untouched; naive one-pass runs violate S2's scenario-last ordering (59× duplicate-table from guard re-entry) and exhaust `max_locks_per_transaction` on a shared cluster (45× at p8). Three constraints make it green: (1) two-pass per shard — non-scenario files then scenario files (1,534 tests clean at p4 in 1m08s; scenario pass 135 tests with only pressure artifacts); (2) CI postgres service needs `-c max_locks_per_transaction=256`; (3) the three scenario anchor groups (07-11 / LabTat 07-12 / PharmacyTat 07-13) must not share one worker process — a growth assertion counted exactly 3× when all three anchors landed in one worker (1008 = 3×336). Also: Q6 NDJSON evidence needs per-worker filenames (two workers appending one file interleave) + a widened manifest-generator glob.* *Consolidation SHIPPED 2026-07-26 (PR #91, squash `84b5f830`): 4 feature shards × 2 paratest workers — 9 → 5 backend jobs; ordered passes per shard (non-scenario → shared-anchor scenario → each anchor-override class solo); `make-paratest-suite.py` generates per-pass phpunit configs (paths absolutized — a phpunit config resolves paths relative to the config file); per-pid NDJSON evidence; 4-bin manifest at 475.0 s each; branch protection trimmed 17 → 13 required contexts (feature-4..7 removed, strict preserved — do NOT re-add). Live on its own PR run: unit 1m17s, shards 4m06s/7m20s/6m15s/6m15s, all green; walls held on subsequent full runs (#94: 4m24s/7m17s/6m16s/6m44s). §3.2.9 parity 1,672 = 1,672 by NDJSON name-diff. Closeout correction (verified 2026-07-27): the spike's `max_locks_per_transaction=256` constraint is deliberately ABSENT from ci.yml — lock exhaustion was observed at p8/p4 on a shared cluster, and the shipped shape (2 workers, dedicated per-job service container) runs green without it across three full runs; it becomes relevant only if `BACKEND_SHARD_WORKERS` is raised.*
...
Budgets (extend §3.2.9's): after S1 lands, ratchet backend `timeout-minutes` 30 → 20; after stage-2 ratification, adopt the ≤15 m backend critical-path budget with the mandated 3-clean-run evidence. Re-run the full 19-job suite with zero skipped tests before/after each tier and diff coverage counts, assertions, and invariants (§3.2.9 requirement) — a faster shard must not conceal a semantic regression. *Closeout status (2026-07-27): the 30 → 20 ratchet shipped (PR #76, after the feature-1 residual was explained with evidence). The ≤15 m backend critical-path budget is now met with ~2× headroom (worst shard ~7.3 m). **RATIFIED [SU] 2026-07-27**: the ≤15 m stage-2 backend critical-path budget is adopted, with the mandated 3-clean-run evidence on the consolidated shape — runs 30201161345 (#91 PR lane), 30230900947 (#93), 30233798984 (#94); worst backend shard 7m20s across all three, and backend lanes are retry-free on every lane so PR-lane backend walls are valid budget evidence.*
...
**PROGRAM COMPLETE 2026-07-26** (verified 2026-07-27). Every item above is shipped, measured-null (S3, PR #77 closed — see §5's do-NOT-do list), or spiked-and-consumed (D3 spike → consolidation PR #91). Final tranche beyond the Tier 2–3 devlog: #84 (§D2), #87 (Radiology/chattiness), #89 (Pint 1.29), #90 (manifest regen), #91 (D3 consolidation). Full record: `docs/DEVLOG-ci-duration-tier1-2026-07-24.md`, `docs/DEVLOG-ci-duration-tier23-2026-07-26.md`, `docs/DEVLOG-ci-duration-closeout-2026-07-26.md`. Headline: PR merge-gating wall ~25–27 m → ~10 m; merge→deployable ~25 m → ~10 s; docs PRs ~19 m → ~2–3 m; backend 9 jobs at 7–25 m coin-flip → 5 jobs at ~4–7 m deterministic.
