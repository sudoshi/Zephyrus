...
- **Required-checks contract:** 17 required contexts exist by *name*. Any job rename/merge/removal must update branch protection in the same change window or PRs become unmergeable (this kills the "delete the DAST job" variant — DAST **is** required). *(Post-#91 update: now 13 required contexts — `Backend tests (feature-4)`..`(feature-7)` were removed with the 8→4 shard consolidation, strict mode preserved. Do not re-add them.)*
