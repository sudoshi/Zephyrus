...
- [x] Run migration lane, contract lane, and full backend suite. Migration lane passes 39 tests / 200 assertions and contract lane passes 59 tests / 1,764 assertions on the Linux host via the wrapper (Bash 5 `mapfile` works); the previously-failing 8 Mobile-parity assertions are now green. Full backend suite (`php artisan test`) passes 1,862 tests / 100,171 assertions with 1 skipped and 0 failures (RouteSmoke covered 124 GET routes with 0 failures; ~58 min, 2026-07-24).
...
- [x] Full backend suite passes: 1,862 tests / 100,171 assertions, 1 skipped, 0 failures (`php artisan test`, ~58 min, Linux/PostgreSQL 17 host, 2026-07-24). No unrelated baseline failures; the additive `001700` migration and new tests regress nothing.
