...
Remaining (Wave 2, not yet built): the **plausibility sampler** that clears the 6 validator WARNINGS (ESI pyramid, differentiated occupancy, discharge-before-noon 18–42%, transport priority mix + overdue share, OR scale) by wiring `DistributionProfile` into the generators. Also open: a retention/prune job for `occupancy_snapshots`/`census_snapshots` and the ledger (Wave 5), and an admin health endpoint + refresh-history panel.
