...
Remaining: the single `info`-level `plausibility.or_room_scale` gap (4 physical ORs vs 44 declared procedure spaces) — needs an OR-domain scale-up (more rooms + a realistic block schedule / case volume), a larger effort. Also open: retention/prune for `occupancy_snapshots`/`census_snapshots` + ledger (Wave 5), and an admin health endpoint + refresh-history panel.
