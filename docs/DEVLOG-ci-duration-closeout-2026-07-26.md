...
- **Staff iOS build-side** (4.8 m Debug build, 2.7 m project verify, 2.1 m Release build) — the only lever that still moves the full-run wall; pending an explicit call. *Closed without action: the optional tranche is declined; reopen only on explicit request.* *REOPENED by [SU] later the same day ("I want those minutes back. I want complete optimization.") and executed as PRs #99/#100 — see `DEVLOG-ios-build-side-tranche-2026-07-27.md` (both iOS jobs split into test + required release siblings, ARCHS=arm64, required contexts 13 → 14).*
