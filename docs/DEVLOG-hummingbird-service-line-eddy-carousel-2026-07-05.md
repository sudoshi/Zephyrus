...
- **Self-healing cache:** added `Spaces3dAssetService::SCHEMA_VERSION = 2`. `load()` rebuilds the on-disk asset when its `schema` differs, so a deploy that adds a field doesn't serve the stale `storage/app/flow/spaces3d.json` forever (the pre-existing file had no service lines and would have been served indefinitely). Version hash now covers spaces + legend.
