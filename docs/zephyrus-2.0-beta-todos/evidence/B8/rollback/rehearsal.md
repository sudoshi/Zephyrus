...
Status: plan documented; production rollback rehearsal pending deploy
...
- This slice adds no Laravel migrations.
- Rollback is expected to be app-artifact/commit based unless production has unrelated pending migrations.
...
- No database rollback is expected for this slice.
- If unrelated pending migrations are found, stop and use the established ops-approved PostgreSQL backup/restore procedure.
- Do not place database credentials in evidence files.

## Rehearsal Gap

This rollback has not yet been rehearsed against production because deployment has not yet completed in this evidence package. Archive the actual rollback artifact or previous known-good commit after deployment.
