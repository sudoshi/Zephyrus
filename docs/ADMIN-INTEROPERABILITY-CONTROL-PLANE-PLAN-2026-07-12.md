...
**Implementation status (2026-07-15):** Phase 0 production safety, the Admin cockpit, enterprise/source scope, governed source lifecycle, source onboarding/readiness/scheduled activation, credential/network authority, operational observability, vendor-neutral FHIR polling, governed resource profiles, and immutable bounded FHIR/SMART discovery evidence are code-complete on `feat/admin-production-safety`. Checked items below mean code-complete with automated evidence on that branch; they do not waive production smoke, institutional provider/bootstrap configuration, raw-PHI storage, partner conformance, or go-live gates.
...
**Capability/SMART discovery tranche (2026-07-15):** every successful health check now normalizes the full bounded FHIR R4 CapabilityStatement server surface and SMART configuration into `integration.fhir_conformance_observations` plus resource-detail children. The append-only, source-bound observation chain records document hashes; FHIR version/status/date/kind; software and implementation origin; formats, patch formats, implementation guides, compartments, security services, system interactions/operations; per-resource base/supported profiles, interactions, versioning, conditionals, search parameters, includes/revincludes, and operations; SMART issuer/JWKS/authorization/token/registration/management/introspection endpoints, grants, client authentication methods/algorithms, scopes, capabilities, PKCE methods, and associated endpoints; and derived batch, transaction, history, search, subscription, and Bulk Data support. Parsing fails closed on non-R4/non-server statements, missing required metadata, unsafe endpoints, unsupported backend-service grants/authentication, interactive authorization without S256, endpoint/credential token drift, malformed arrays, or the explicit 500-resource, 500-search-parameter-per-resource, and 100-operation-per-resource bounds. PostgreSQL rejects history rewrites, pointer rewind/clear, count-inconsistent finalization, mismatched source authority, late child insertion, and recognizable clinical content. Only searchable discovered resources become capability/profile inputs. A source-scoped read API and strict React contract render PHI-free counts, hashes, endpoint origins, flags, warnings, and up to 100 resource rows; they render unobserved/loading/error states truthfully and never expose full discovered URLs. Browser validation also drove an Inertia-safe tab-history fix so clearing Admin scope preserves `tab=fhir`. The final authoritative gates pass **1,139 backend tests / 17,476 assertions** with one intentional skip, **105 Vitest files / 451 tests**, TypeScript, Laravel Pint across **1,257 PHP files**, `git diff --check`, Composer validation, and the production Vite build. The complete fresh-migration authenticated Chromium lane passes **33 tests** with three deliberate feature skips; evidence is retained at `artifacts/release-evidence/browser/20260716T012746Z-browser-playwright.json`, and its disposable database was removed. Remaining FHIR work is the unchecked search/write/history semantics, resource mappers and validator packages, actual Bulk Data and subscription execution, and source reconciliation. Discovery evidence identifies advertised support; it does not claim those workflows are implemented or partner-certified. Production was not changed.
