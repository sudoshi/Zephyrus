...
4. Neither staff client consumes the existing refresh-token endpoint. Android's forced-password-change path is now native and device-verified, including atomic backend rollback on session-revocation failure; refresh coordination remains incomplete.
...
| Staff native communication         | iOS and Android implement capability-gated inbox/detail plus all six mutations, exact explicit replay, authorization/omission privacy purging, retained-row transition reconciliation, restricted For You cards/deep links, and content-free possible-reroute recovery                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | iOS passes **60/60** full unit and **16/16** full simulator UI tests plus an optimized Release build and scoped privacy/persistence scans; Android passes **92/92** Debug and **89/89** Release JVM tests, lint/assembly, and **16/16** full API 35 instrumentation tests                                                                                                                                            | No push, authoritative service-ownership integration, or live deployed backend E2E; wider legacy staff workflows need comparable test depth                                                                            |
...
| ---------------------- | ---------------------------------------------------------------------------------- | --------------------------------------------------------------------------- | --------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Staff token login      | Sanctum issue/revoke/refresh/change-password                                       | Login, Keychain, biometric lock                                             | Login, encrypted preferences, biometric lock                                | `PARTIAL`: refresh remains unused; Android password change is native and device-verified                                                                                            |
...
- [ ] Make token refresh proactive and single-flight on both clients; retry only idempotent reads automatically.
...
- [x] Expand `MobilePatientContextService::userSharesPatientUnit()` to derive active patient unit from `prod.encounters` and other authoritative current-location sources, not only ED visits and EVS requests. _(2026-07-19: active, non-deleted encounter units now participate in unit-scope authorization.)_
...
- [ ] Implement single-flight refresh on 401/near-expiry in iOS and Android.
- [ ] Rotate refresh tokens on use; revoke the predecessor; detect reuse; revoke the token family on suspected theft.
...
- [ ] Implement single-flight token refresh parity on both staff clients.
...
- [ ] Link a patient principal, grant, approved projection, and separate iOS/Android enrollment challenges to the synthetic encounter. _(2026-07-21 remote read-only verification reconfirmed the single active, command-owned source encounter `10040` in unit `85` and all four required `patient_experience` identity tables. The local runtime is correctly disabled for reference provisioning and does not hold the patient HMAC; encrypted linkage therefore remains pending execution from the deployed application with its actual APP_KEY and patient HMAC secret.)_

**Controlled deployment procedure — reference-patient identity (pending):**

1. Publish the reviewed, merged release through `./deploy.sh` from the canonical `/home/smudoshi/Github/Zephyrus` checkout only. The script refuses a noncanonical path, a dirty worktree, a branch/upstream other than `main`/`origin/main`, or a local/remote SHA mismatch. Do not substitute an SSH command block, direct production `git pull`, direct database SQL, or a checkout copy for this release gate.
2. On the approved deployed application runtime only, confirm migration batches 38/39, a non-placeholder `APP_KEY`, `HUMMINGBIRD_PATIENT_HMAC_SECRET`, `HUMMINGBIRD_PATIENT_REFERENCE_ENCRYPTION_KEY_VERSION`, and the explicit reference-provisioning flag are present. Do not copy any of those values to a local checkout, shell history, ticket, or chat.
3. Run `php artisan hummingbird:provision-reference-patient-identity --patient-ref=demo-hummingbird-reference-inpatient --json` without `--commit`. Confirm that it resolves only encounter `10040` and plans one command-owned pending principal, verified identity link, access grant, and one challenge for each platform.
4. After the designated security/privacy owner approves the dry run, run the same command with `--commit` but without `--show-secrets`. Confirm its redacted result, immutable audit records, and the expected source/link/grant relationship through a read-only operational check.
5. Deliver the two one-time enrollment challenges through the approved secure enrollment workflow, not terminal logs or chat. Verify activation, revocation, and redacted audit behavior through the deployed patient API before a clinical pilot.
6. Disable the reference-provisioning flag immediately after the controlled exercise and retain only the required operational audit evidence.
