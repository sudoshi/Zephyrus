...
- [x] Regenerate shared JSON fixtures from server factories and decode them in PHP, Swift, and Kotlin CI. _(2026-07-25: the eight staff artifacts are captured from real factory-seeded BFF responses, with exact endpoint/test provenance in `fixtures/fixture-provenance.v1.json`. The six patient-care artifacts—Today, My Path, events, discharge readiness, rounds summary, and Care Team—are separately captured through real patient BFF routes from the deterministic **testing-only** synthetic projection provisioner, with their boundary recorded in `fixtures/patient/fixture-provenance.v1.json`; they are not production source or release evidence. The default PHP test compares each checked-in patient fixture with its deterministic BFF response and fails stale artifacts without writing (**1 test / 72 assertions**); the fixture validator adds envelope, PHI-minimization, native-harness, and provenance checks (**3 tests / 128 assertions**). iOS decodes all six in `PatientAPIModelTests`, Android decodes all six in `PatientProjectionFixtureDecodeTest` (**6 tests**). Capturing exposed a real staff boundary defect—`isolation_required: "none"` versus the Boolean native contract—which `MobilePatientContextService` now normalizes under a non-dump BFF regression (**2 tests / 26 assertions**). iPhone 17 Pro Simulator passed the six-fixture XCTest and the patient UI journey; Android API 35 emulator passed the seven-test patient primary journey. The separate generated Swift/Kotlin model artifact remains open.)_
