...
The first-stage target is ratified across three clean complete runs:

| CI evidence                                         | `feature-6` | `feature-7` / critical path | `feature-7` reduction | Result |
| --------------------------------------------------- | ----------: | --------------------------: | --------------------: | ------ |
| Preliminary PR run `30122622952`, head `a6db752b`   |      15m35s |                      24m56s |                 65.8% | 19/19  |
| Rebased PR run `30124481668`, head `26986697`       |      17m07s |                      26m51s |                 63.2% | 19/19  |
| Exact-merge run `30126216355`, `main` at `f148bc81` |      17m55s |                      24m15s |                 66.7% | 19/19  |

Every run reduced `feature-7` by at least **60%** from the 1h12m54s whole-job baseline and kept every shard below **30 minutes**. The exact-merge JUnit evidence preserved **224 tests / 2,755 assertions** in `feature-6` and **194 tests / 8,881 assertions** in `feature-7`, with zero failures, errors, or skips. This closes only the first-stage ceiling; the **15-minute** critical-path target remains open.

Residual JUnit evidence identifies the next measured work instead of hiding it behind sharding: `AncillaryDemoScenarioTest` is **753.74 seconds / 52.0%** of the first optimized `feature-7`; `PharmacyDispenseTest` is **365.69 seconds / 41.5%** of the first optimized `feature-6`; the two flow-board classes and pharmacy TAT analytics are approximately stable. The remaining work must profile those paths and test setup directly before introducing a weighted manifest or any index.

PR `#60` merged as `f148bc81e458f665cbed5878c555697041a25d92`. Canonical `./deploy.sh --check` initially failed closed because the server checkout was parked on a fully pushed feature branch. The feature tip remained preserved on its local and remote feature refs; only the clean server `main` pointer was realigned to the already-fetched `origin/main`. The repeated preflight then passed, and canonical `./deploy.sh --confirm f148bc81e458f665cbed5878c555697041a25d92` built and synchronized an immutable release, refreshed permissions and Laravel caches, restarted the queue worker, Arena, and Apache, reverified edge security, and completed all final vhost checks. Post-deploy reconciliation confirmed the release marker and canonical checkout at the exact merge SHA, a clean checkout, active Apache and queue-worker services, healthy disabled-by-policy Arena, and an external HTTP `302` from the Zephyrus vhost. No migration, pathway approval/release, patient activation, session, token, or feature-gate change was part of this release.
...
- [x] Ratify the first-stage performance budget across three clean complete runs: each records at least a **60%** reduction from the 1h12m54s `feature-7` whole-job baseline, no shard above **30 minutes**, all 19 jobs green, and preserved critical-shard tests/assertions with zero skips.
- [ ] After residual query/setup remediation, tighten the backend feature critical path to **15 minutes** across three clean runs. A budget change requires recorded evidence and review; a timeout increase is not optimization.
- [x] Complete final publication evidence on the rebased implementation head and exact squash merge: every backend shard, capability/contract/security verifier, browser lane, and all four native products passed in both 19-job runs with no retry.
- [x] Deploy only the exact CI-successful merge through the canonical preflight/release path and independently reconcile its release marker, clean canonical checkout, supervised services, sidecar health, and external vhost response. No migration or patient/clinical gate changed.
