...
3. iOS has a more complete Eddy entry experience and push implementation. Android now supports scoped, nonstreaming staff Eddy chat from its authorized context screen, and both native clients present authorized, no-store read-only server conversation history plus a server-governed approval inbox, fetch-on-open dry run, and explicit online human decision. Neither client has mobile streaming, history deletion, or an Android global Eddy entry, and Android has notification channels but no complete FCM registration/delivery path.
...
| Eddy staff             | Chat, stream, conversations, approvals, context                                                                                                       | Chat, context, no-store read-only history, and governed approval review/decision | Context, scoped nonstreaming chat, no-store read-only history, and governed approval review/decision | `PARTIAL`: history and approvals are server-scoped/no-store on both clients; Android still lacks iOS's global entry, and neither client has streaming or history deletion             |
...
| Eddy chat/conversations                           | Both native clients have authorized, no-store read-only history; Android chat remains scoped/nonstreaming                                         | `NATIVE`                      | P1: global entry parity, streaming fallback, conversation retention/deletion policy   |
