...
3. iOS has a more complete Eddy entry experience and push implementation. Android now supports scoped, nonstreaming staff Eddy chat from its authorized context screen, and both native clients now present authorized, no-store, read-only server conversation history. Neither client has mobile streaming or an approval-view/decision workflow, and Android has notification channels but no complete FCM registration/delivery path.
...
| Eddy staff             | Chat, stream, conversations, approvals, context                                                                                                       | Chat, context, and no-store read-only history                               | Context, scoped nonstreaming chat, and no-store read-only history                       | `PARTIAL`: history is user-scoped/read-only on both clients; Android still lacks iOS's global entry, and neither client has streaming or mobile approval view/decision completion     |
...
- [ ] Complete Android Eddy chat/conversations/approvals.
    ...
    - [x] Add authorized, server-owned, no-store read-only conversation history to both native clients. _(2026-07-24: list/detail routes are user-scoped and exact-schema documented; Android exposes history from the authorized context chat and iOS exposes it from the chat toolbar. Neither client persists transcripts, queues an Eddy request offline, or offers an approval control. The BFF isolation suite passes 8 tests/44 assertions; Android JVM and full API 35 emulator instrumentation pass, including two history UI tests; iPhone 17 Pro Simulator tests prove no-store transport for context, chat, list, and detail plus safe decoding of null draft actions. Streaming, deletion, approval view/decision, and Android global entry remain open.)_
