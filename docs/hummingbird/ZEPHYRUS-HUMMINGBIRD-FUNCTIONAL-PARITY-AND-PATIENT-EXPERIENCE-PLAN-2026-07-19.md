...
2. iOS and Android share vocabulary and much of the visible product grammar, but still duplicate DTOs, client behavior, caching, authorization interpretation, and business rules. Generated platform-native contract artifacts are the accepted target, but that generator and migration are not complete.
3. iOS retains richer active-screen Eddy context and push implementation. Android now has a persistent global Eddy entry that opens only the server-authorized house lens, plus its scoped, nonstreaming context chat. Both native clients present authorized, no-store read-only server conversation history plus a server-governed approval inbox, fetch-on-open dry run, and explicit online human decision. Neither client has mobile streaming or history deletion, and Android has notification channels but no complete FCM registration/delivery path.
4. Staff session rotation and self-service revocation are now cross-platform and server-governed: both clients use a process-wide single-flight coordinator, Laravel maintains stable refresh-token families with one-time generations, reuse-theft detection, family-scoped revocation, and absolute expiry, and staff can inspect a privacy-minimized device list and revoke an exact owned family. Device attestation, push-device correlation, background timeout policy, and high-risk reauthentication remain incomplete.
...
- The capability ledger currently inventories **60 staff mobile/auth operations** with a single owner, including all nine accountable patient-communication operations and both staff self-service session operations.
...
| Staff native communication         | iOS and Android implement capability-gated inbox/detail plus all six mutations, exact explicit replay, authorization/omission privacy purging, retained-row transition reconciliation, restricted For You cards/deep links, content-free possible-reroute recovery, and process-wide proactive/single-flight staff-token rotation. A 401 is automatically retried once only for a GET; mutations may refresh before first transmission but are never replayed after a 401. Laravel binds every issued generation to a stable server-side family and revokes that family on predecessor reuse.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | iOS passes **80/80** full unit tests, the complete **18/18** simulator UI matrix including the two session-management journeys, and an optimized Release build; Android passes **109/109** Debug and **106/106** Release JVM tests, lint/Debug+Release assembly, and the complete **20/20** API 35 instrumentation matrix including the two session-management journeys. The native suites include terminal server reuse handling without diagnostic leakage, safe self-service revocation, strict contract decoding, and largest-text/reduced-effects coverage. | No push or authoritative service-ownership integration; no device attestation, deployed staff-login E2E, or wider legacy staff workflow depth                                                                          |
...
| Contract and CI controls           | Capability ledger, generated report, staff OpenAPI verifier, patient OpenAPI verifier, disclosure-matrix verifier, patient product-boundary scanners, and independent patient iOS/Android CI jobs                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Verifiers own **52 capabilities / 101 navigation routes / 60 staff mobile-auth operations / 25 patient operations**, all **nine** staff communication operations, both staff session operations, and **11/14/3** disclosure controls; targeted Pint passes; CI has no deployment step                                                                                                                                                                                                                                                                            | Normalize path casing and observe clean Linux CI; CI has not run remotely for this uncommitted work; governance review cannot be replaced by CI                                                                        |
...
- Every one of the **60** registered staff mobile/auth operations now carries governed authorization, request/response/storage data classification, idempotency/retry, and error-envelope/status behavior in `hummingbird-bff.v1.yaml`. The verifier compares the complete OpenAPI operation inventory with Laravel, requires `mobile:read` on all 56 mobile BFF operations, and proves `mobile:act` declarations match route middleware exactly. Refresh-token authentication is explicitly bearer-token protected in the contract.
...
#### 3.2.6 Staff self-service device and session management — 2026-07-23

- [x] Add governed `GET /api/mobile/v1/me/sessions` and `DELETE /api/mobile/v1/me/sessions/{sessionUuid}` operations. Both require the active, unexpired, server-issued `mobile-access:<family UUID>` credential whose exact Sanctum row ID matches the lifecycle row's current-access pointer, plus its `mobile:read` ability; a browser cookie, Sanctum transient principal, refresh/legacy token, unrelated personal token, or adversarial same-family-name token cannot enumerate, observe, or revoke mobile families. Migration backfill binds only an unambiguous single live predecessor access row and otherwise fails closed. The immutable operation baseline now owns **85** staff-and-patient operations, and route/OpenAPI parity owns **56** staff mobile BFF routes.
- [x] Enforce active staff-account state again at the mobile BFF boundary so a stale or concurrently authenticated access token fails closed even if it outlives deactivation. Keep that guard off the refresh route: refresh must reach its lifecycle controller so an inactive-account attempt revokes the complete token family and records the governed `account_inactive` reason.
- [x] Bind newly issued staff families to an exact, optional device object containing a canonical installation UUID, `ios`/`android` platform, bounded device name, app version, and OS version. The server—not the client—sets the environment, and refresh preserves the original binding. The installation identifier is a client-generated correlation value, not a credential, proof of possession, hardware attestation, or authorization claim.
- [x] Return only bounded, active, owned families and always retain the current family within the 100-item response ceiling. The projection deliberately excludes installation UUID, token-family UUID, token row IDs, bearer/hash material, IP/network data, user agent, patient data, and free text. Another user's UUID and an unknown UUID return the same generic `404`.
- [x] Make session responses `private, no-store, max-age=0` with `Pragma: no-cache` and `Expires: 0` across successful, unauthenticated, authorization-denied, malformed-identifier, not-found, and exception paths. Both native clients also disable HTTP caching and send no-store request directives for the inventory and revocation routes.
- [x] Preserve the token-row → family-row lifecycle lock order during self-service revocation. Exact-family deletion is idempotent; current-family revocation invalidates both access and refresh credentials; remote-family revocation leaves all other families usable. Last-seen observation is rate-limited to one write per five minutes and is never treated as authorization, identity proof, or presence.
- [x] Make successful inventory disclosure fail closed if its access audit cannot be written. Make revocation and its `mobile.auth.session_revoked` audit event one database commit boundary, so an audit outage rolls the token-family mutation back. Denied unknown/cross-user requests retain indistinguishable public behavior and content-minimized best-effort audit.
- [x] Add native staff surfaces on both platforms under Profile. iOS uses the shared calming Hummingbird backdrop; Android uses the established staff design system. Both identify the current device, show only safe recognition metadata, require destructive confirmation, refetch after an idempotent remote result, clear protected credentials and user-scoped caches after current-session revocation or terminal auth rejection, and avoid blind mutation replay.
- [x] Add cross-platform regression evidence. A clean remote Laravel worktree passes **83/83** focused auth, session-management, backend-safety, account-lifecycle, identity-schema, and route-contract tests with **1,326 assertions**, plus an exact two-route session inventory. iOS passes **80/80** full unit tests, a focused **2/2** session UI suite at normal and accessibility stress settings, the complete **18/18** simulator UI matrix, and optimized Release compilation; a byte-level scan of the Release `.app` confirms that the session UI-test flags, in-memory bearer marker, fixture device names, and fixture session identifier are absent. Android passes **109/109** Debug and **106/106** Release JVM tests, lint, Debug/Release assembly, the focused **2/2** session instrumentation suite, and the complete **20/20** API 35 instrumentation matrix. Tests cover exact request shape, durable identity persistence/failure, privacy-safe and structurally strict decoding, canonical identifiers, list-wide uniqueness/current-session invariants, bounded Unicode metadata, no-store behavior, no DELETE idempotency header, response-to-request UUID binding, confirmation, authorized-refetch-without-mutation-replay, current/remote terminal policy, largest text, reduced effects, and fractional UTC Laravel timestamps.
- [ ] Do not treat this slice as device-management completion. It does **not** yet provide attestation, token fingerprint projection, push-registration correlation, uninstall feedback, administrator-assisted lost-device recovery, federated session control, MFA/passkeys, high-risk step-up, or a production timeout/biometric-grace policy.

#### 3.2.7 Four-product transport security and staff native CI — 2026-07-24

- [x] Establish one explicit release transport boundary across Hummingbird Staff iOS/Android and Hummingbird Patient iOS/Android. API traffic is HTTPS-only to `zephyrus.acumenus.net` on the standard TLS port; staff realtime is WSS-only to that same host on port 443. Each runtime rejects credential-bearing URLs, a path prefix, query, fragment, wrong scheme, wrong production host, invalid or nonstandard production ports, and every redirect before a request or bearer credential can be replayed.
- [x] Remove the release ATS exception from both iOS products. Staff iOS now evaluates `DEBUG` before simulator selection, so a Release simulator exercises the production origin instead of silently selecting local HTTP. A staff Debug simulator may use localhost; a Debug physical device defaults to production and accepts only a system-trusted HTTPS/WSS override. Patient iOS remains HTTPS-only in every build.
- [x] Replace staff Android's application-wide debug cleartext switch with declarative Network Security Configurations. Both Android products deny cleartext in their base policy and trust only system roots; the staff Debug overlay permits cleartext only to `localhost`, `127.0.0.1`, and emulator alias `10.0.2.2`. The patient product has no cleartext override.
- [x] Make and document the production pinning decision in `ADR-2026-07-24-mobile-transport-security-and-pinning.md`. The live endpoint observed on 2026-07-24 used a short-lived Let's Encrypt `YE1` certificate valid through 2026-09-26. Static certificate/public-key pins are deliberately **not** shipped until backup identities, overlap rotation, forced-upgrade/kill-switch recovery, stale-client drills, monitoring, 24x7 ownership, and an incident runbook exist. The decision has a 2027-07-24 maximum review date and earlier PKI/hostname/threat-model triggers.
- [x] Add `scripts/verify-hummingbird-transport-security.php` to backend quality CI. It checks both iOS project sources, both Android manifests and main trust policies, the exact staff Debug allowlist, all four production-host policy sources, redirect refusal, absence of ungoverned pin/permissive-trust hooks, and the bounded ADR review decision. Separate artifact verifiers inspect each built Release iOS Info.plist plus executable origin strings and each Android manifest plus its compiled Network Security Configuration rather than inferring release posture only from source.
- [x] Promote both staff apps to first-class native CI products. New macOS and Ubuntu jobs verify the generated staff Xcode project, compile Debug and Release, run full unit/lint/instrumentation/UI gates, inspect Release transport artifacts, and retain evidence. The Swift job directly decodes all eight shared DTO fixtures; Kotlin decodes the same fixtures in its required unit suite; PHP continues to validate them in the backend shard. This closes the three-language **fixture-decode CI** sub-control, but it does not claim generated DTO completion or server-factory provenance for all fixtures.
- [x] Record clean local native evidence with no skips: staff iOS **86/86 unit** and **18/18 UI** tests on an iPhone 17 Pro simulator; staff Android **115/115 Debug JVM**, **112/112 Release JVM**, lint, Debug/Release assembly, and **20/20** instrumentation tests on an API 35 emulator; patient iOS **65/65 unit** and **8/8 UI** tests on the same simulator; and patient Android **93/93 Debug JVM**, **88/88 Release JVM**, lint, Debug/Release assembly, and **15/15** instrumentation tests on the same emulator. The new negative cases cover malicious host/scheme/port/path/query/fragment/user-info inputs and redirect refusal. Both generated Xcode projects match their specifications, all four Release artifacts pass the transport inspectors, and the eight-fixture Swift harness passes.
- [x] Make staff iOS authorization-loss UI fixtures deterministic under loaded Xcode runners. The 401/403/404 and authorized-inbox-omission journeys now seed their sentinel only through the existing explicit Debug UI-test process boundary, assert that the in-memory draft is present before denial, and then verify the security transition purges the editor, sentinel, routing commands, and—when required—the session. This removes the race in which short authorization polling could erase the editor while XCUITest was still injecting text; the complete **11/11** routing class and optimized Release compilation pass locally, and the hook is compiled out of Release.
- [x] Reinspect the patient presentation after transport hardening. The current iOS synthetic-reference home renders the app-local Hummingbird photograph as a soft full-screen background under deterministic scrims and translucent, high-legibility cards; it keeps the synthetic-data notice, freshness/source provenance, care-plan introduction, and urgent-help guidance visible without implying live clinical data. The Android API 35 activity deliberately produces a black host screenshot because `FLAG_SECURE` is active; the full **15/15** instrumentation suite verifies the branded reference journey and secure-window policy in-process. This reconfirmation is regression evidence only: the image source/license/attribution production hold and the formal contrast, reduced-transparency, large-text, screen-reader, patient/family, and clinical-safety acceptance gates remain open.

#### 3.2.8 Synthetic reference-patient draft-projection gate — 2026-07-24

- [x] Reconcile the production reference foundation before adding projection state. A content-minimized read-only audit found exactly one command-owned active synthetic encounter (`10040`) in active unit `85` (`5 East — Medical/Surgical`), exactly one command-owned pending/inactive and passwordless patient principal, one verified synthetic identity link, one pending self grant, two issued and unconsumed enrollment challenges, no patient sessions or bearer tokens, and **zero draft or released projections**. No challenge, credential, HMAC, encryption key, patient display name, or projection content was read or emitted.
- [x] Refresh only the command-owned operational sample row under an explicit guarded PostgreSQL transaction. The transaction took a reference-patient advisory lock, asserted exactly one owned encounter and exactly one active target unit, then preserved encounter `10040` as active, not deleted, not discharged, in unit `85`, with no bed assignment and an expected-discharge date of `2026-07-26`. The write affected exactly one row and preserved the command ownership markers. The local Laravel seed path was not pointed at production: it correctly refused before writing because the local runtime does not possess the dedicated production patient-context signing key.
- [x] Implement a dry-run-first production-runtime command for six **draft-only** synthetic reference projections: `today`, `pathway`, `pathway_events`, `discharge_readiness`, `rounds_summary`, and `care_team`. `hummingbird:draft-reference-patient-projections` requires the exact `--commit --confirm-draft-only=synthetic-drafts-not-patient-visible` acknowledgement, an existing exact command-owned encounter, the exact command-owned pending identity/grant/link, an active operational unit, and the absence of every patient session and Sanctum token. A local runtime refuses a remote database before any projection query or write.
- [x] Keep the fixture outside every patient disclosure and publication path. The command creates one deterministic draft release-policy version with no approver or effective window, six deterministic draft cursors, and six sequence-one projections marked `draft_synthetic_not_approved`. It cannot activate a principal or grant, issue or expose a credential, approve a policy, create a released projection, or publish an outbox fact. Command output contains statuses and counts but no projection content or principal/grant/encounter UUID. Creation and replay both run the patient projection content guard over content, provenance, uncertainty, and relationship fields, so later drift in the shared reference fixture cannot bypass the production allowlist. Governance JSON comparisons are key-order-neutral but value-type-strict, preventing numeric lookalikes from satisfying boolean safety rules. Exact replay verifies the complete stored content digest, schema, provenance, uncertainty timestamp, scope, relationship, source time, cursor digest/metadata, policy rules, and absence of any release outbox fact; partial, altered, ambiguous, active-access, unsafe-content, loosely typed, or previously published state fails closed.
- [x] Revalidate the surrounding backend and both independent patient products locally. The complete Patient feature suite passes **186/186 tests and 3,147 assertions** on an isolated PostgreSQL 16 instance, including dry run, exact acknowledgement, deterministic replay, append-only governance, draft non-disclosure even after adversarial activation of an unrelated policy, shared-content allowlist drift, loose JSON type equivalence, active identity, session/token, unexpected publication fact, local-to-remote refusal, and released-projection outbox behavior. Patient iOS passes Debug and optimized Release builds, Release product/transport inspection, **65/65 unit** and **8/8 UI** tests on an iPhone 17 Pro simulator; reviewed Today and My Path attachments retain the app-local Hummingbird imagery, explicit synthetic banner, provenance/freshness, uncertainty, and urgent-help hierarchy. Patient Android passes source/product boundaries, **93/93 Debug** and **88/88 Release JVM** tests, both lint variants, both APK assemblies, Release product/transport inspection, and **15/15 API 35** instrumented tests; a manual host screenshot is completely black as required by `FLAG_SECURE`.
- [x] Deploy and independently verify the draft-projection implementation without opening a patient disclosure path. PR `#56` remained pinned to head `bb95e49ce6ab298a084953c9460ab1205fab4310`; exact-head CI run `30107421291` completed **19/19** successful jobs after a same-SHA retry of an external DAST artifact-upload failure, with the pinned ZAP scan itself successful in both attempts. The squash merge produced `f7074936a2f265fbfd785f69c5f30632503e2fe8`; exact-merge `main` CI run `30112436573` then completed **19/19** successful jobs. The first canonical `./deploy.sh --check` correctly stopped because the clean server checkout had been left on a fully pushed feature branch. After a branch-only switch back to its tracked `main`—without a direct production pull or substitute deployment—the second preflight passed and canonical `./deploy.sh` completed with release marker `f7074936a2f265fbfd785f69c5f30632503e2fe8`, edge-security verification, cache clearing, queue-worker installation/restart, Arena sidecar health, Apache restart, and final vhost checks.
- [x] Execute the bounded production draft transaction and prove exact replay. On the deployed runtime, the encounter-`10040` dry run returned `create_six_owned_reference_drafts`, zero existing projections, pending/inactive access, and zero sessions/tokens. The exact acknowledged commit returned `created_six_owned_reference_drafts` with `created=6`; an immediate identical commit returned `replayed_six_owned_reference_drafts` with `created=0` and `replayed=6`. All three outputs stated `patient_visible=false`, emitted no content or identifiers, and kept the principal and grant pending.
- [x] Reconcile the post-write state through an independent, content-free direct PostgreSQL audit. The audit confirmed the exact active command-owned encounter in active unit `85`, expected-discharge date `2026-07-26`, one safe pending/inactive passwordless/emailless/phoneless principal, one safe verified synthetic identity link, one safe pending self grant with the exact three read scopes, one exact draft policy with no approver or effective window and typed `patient_visible=false` / `clinical_use=false` rules, the exact six draft projection kinds, six distinct matching cursors, zero release outbox facts, zero sessions, zero Sanctum tokens, and the two pre-existing issued/unconsumed/unrevoked challenges. The deployed runtime separately reported patient product `false`, enabled patient-feature count `0`, reference provisioning `false`, patient care pathways `false`, staff messaging `false`, and rounds-question bridge `false`. An external unauthenticated `GET /api/patient/v1/me` returned HTTP `404` with the generic `not_found` error inside the standard patient-safe envelope and the required no-store/security headers.
- [ ] Do **not** approve or release the six synthetic drafts from this engineering checkpoint. Independent clinical/content review and independent release authority remain required; patient product, pathway, messaging, reference-provisioning, and source-assignment gates remain disabled. Draft creation is implementation and review-fixture evidence, not patient-facing availability, clinical approval, enrollment, or pilot authorization.
...
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Staff token login      | Sanctum issue/revoke/refresh/change-password, stable family lifecycle/reuse detection, privacy-minimized inventory, and exact owned-family revocation | Login, Keychain, biometric lock, single-flight rotation, signed-in devices                | Login, encrypted preferences, biometric lock, single-flight rotation, signed-in devices                             | `PARTIAL`: rotation/reuse and self-service session parity are verified; attestation, push-device correlation, background timeout, high-risk reauthentication, and deployed E2E remain                                                                                                        |
...
| Eddy staff             | Chat, stream, conversations, approvals, context                                                                                                       | Streamed chat, context, no-store read-only history, and governed approval review/decision | Global authorized-house and scoped streamed chat, no-store read-only history, and governed approval review/decision | `PARTIAL`: streamed chat, history, and approvals are server-scoped/no-store on both clients; history deletion, role-by-role deployed evidence, and Android active-screen context parity remain open. Android’s global entry deliberately exposes no active-screen or patient-message context |
...
- [x] Add environment-aware certificate trust policy, transport security tests, and production endpoint pinning decision. _(2026-07-24: exact HTTPS/WSS release origins, default iOS ATS, Android system-root-only Network Security Configurations, bounded staff Debug loopback exceptions, runtime allow/deny matrices, unconditional API/realtime redirect refusal, four Release-artifact inspection gates, an executable cross-product CI verifier, and the explicit time-bounded no-static-pin ADR are complete; full local simulator/emulator evidence is recorded in §3.2.7.)_
...
- [x] Regenerate `mobile-route-contract-inventory.md`; its July 2 snapshot no longer reflects all Flow and Eddy routes. _(2026-07-23: `scripts/generate-hummingbird-route-inventory.php` deterministically renders the live `/api/mobile/v1/*` route table, now **56 routes** after adding the two staff self-service session operations. A `--check` freshness gate runs in CI alongside the capability-report check; the authoritative route↔OpenAPI parity guard remains `MobileBffTest`.)_
...
| Eddy chat/conversations                           | Both native clients have authorized, no-store history and scoped streamed chat; Android also has a global authorized-house entry                  | `NATIVE`                      | P1: active-screen context parity, conversation retention/deletion policy, and role-by-role deployed evidence |
| Eddy approval inbox                               | Both native clients now provide a no-store list, fetch-on-open dry run, explicit online human decision, and server-enforced persona/ability check | `NATIVE`                      | P1: role-by-role live evidence, no autonomous approval, and no offline queue                                 |
...
- [x] Seed the ledger from `navigationConfig.ts`, `routes/web.php`, and `routes/api.php`; review manually for non-navigation capabilities and background jobs. _(2026-07-24: validator proves single ownership for 101 navigation routes and 60 staff mobile/auth operations; non-navigation foundations, all nine staff communication operations, and both staff session operations are included.)_
...
- [x] Add CI that fails when an OpenAPI operation is missing an authorization, data-classification, idempotency, and error-behavior extension. _(2026-07-23: all 60 registered staff mobile/auth operations carry the four governed extensions. `verify-hummingbird-staff-contract.php` checks every operation, validates extension structure and documented error coverage, compares the complete contract inventory with Laravel, and reconciles `mobile:read`/`mobile:act` metadata with actual middleware. CI also runs a negative self-test that removes each extension in turn and proves the gate fails.)_
...
- [x] Add `ETag`/`If-None-Match` or version cursor behavior to high-volume read surfaces. _(2026-07-25: staff Flow Window uses a server-constrained ISO-8601 `since` delta cursor for its high-volume timeline/census refresh: append-only events and snapshots are filtered strictly after the cursor, while current projections, spaces, bed statuses, and duties are served in full; invalid or out-of-window cursors fail closed with `422 invalid_since`. The static, non-live `/flow/floors` and `/flow/spaces3d` geometry assets additionally return strong `ETag` values and honor `If-None-Match` with `304`. Laravel verifies both mechanisms and the redaction/layer invariants (**22 tests / 865 assertions**); Android validates the same merge/cursor behavior in Debug and Release (**8/8** each, plus **5/5** cache-logic tests); the iPhone 17 Pro simulator validates iOS deduplication/replacement semantics and exact cursor wire behavior (**3/3**); and the Android API 35 emulator passes its **27/27** staff regression suite. This bounded staff cache/delta control does **not** relax the patient BFF's private no-store policy or create a patient offline cache.)_
...
- [x] Regenerate shared JSON fixtures from server factories and decode them in PHP, Swift, and Kotlin CI. _(2026-07-25: the eight staff artifacts are captured from real factory-seeded BFF responses, with exact endpoint/test provenance in `fixtures/fixture-provenance.v1.json`; the default Flow/shared PHP capture tests compare each checked-in staff fixture with its deterministic BFF semantics and fail stale artifacts without writing (**7 tests / 170 assertions**, including PHP fixture/provenance validation). Only documented database-sequence identifiers and catalog hashes are canonicalized after grammar validation; status, authorization, relationship, and user-visible payload changes remain exact. The six patient-care artifacts—Today, My Path, events, discharge readiness, rounds summary, and Care Team—are separately captured through real patient BFF routes from the deterministic **testing-only** synthetic projection provisioner, with their boundary recorded in `fixtures/patient/fixture-provenance.v1.json`; they are not production source or release evidence. Its default PHP test also fails stale fixtures without writing (**1 test / 72 assertions**); the patient fixture validator adds envelope, PHI-minimization, native-harness, and provenance checks (**3 tests / 128 assertions**). iOS decodes all six patient fixtures in `PatientAPIModelTests`; Android decodes all six in `PatientProjectionFixtureDecodeTest` (**6 tests**). Capturing exposed a real staff boundary defect—`isolation_required: "none"` versus the Boolean native contract—which `MobilePatientContextService` now normalizes under a non-dump BFF regression. iPhone 17 Pro Simulator passed the six-fixture XCTest and the patient UI journey; Android API 35 emulator passed the seven-test patient primary journey. The separate generated Swift/Kotlin model artifact remains open.)_
- [x] Include nullability, unknown-enum, additive-field, precision, and large-payload fixtures. _(2026-07-25: `patient-pathway-events-forward-compatible.json` is a deterministic, **test-only** derivation of the testing-runtime patient BFF pathway-events capture, explicitly labelled as such in its provenance. It carries an explicit `null` detail, `future_navigation` category, additive `data`/`meta`/`links` objects, the exact JSON integer `9007199254740993`, a decimal retained as a string, and 256 notices. PHP confirms every invariant and stale-file comparison; iOS maps the unknown category to the generic `Care update` vocabulary while retaining the exact integer; Android renders the unknown category through its generic `Status being confirmed` vocabulary and retains the exact `Long`. No raw future value is rendered to a patient. This is client-forward-compatibility evidence only, not an approved clinical source, patient data, or release fixture.)_
...
- [ ] Bind device registration to current user, app, platform, environment, installation ID, token fingerprint, and last-seen time. _(Partial 2026-07-23: each new staff token family is user-owned and records an optional installation UUID, exact native platform, bounded app/OS/device metadata, server-owned environment, and rate-limited last-seen observation. The staff app realm is implicit in the endpoint. Hardware/app attestation, token fingerprint projection, and correlation with the separate push-device registry remain deliberately incomplete.)_
...
- [x] Add a session-management screen for the user to view and revoke their own devices. _(2026-07-23: Laravel exposes privacy-minimized owned active families with generic cross-user/unknown behavior, no-store responses on every matched path, transactionally durable audit, and exact idempotent family revocation. iOS and Android provide confirmed current/remote revocation, terminal local erasure, safe metadata only, and unit plus simulator/emulator accessibility evidence; see §3.2.6.)_
...
- [ ] Today shows the date, care stage, care team, goals, expected rounds window if approved, tests/procedures, mobility/nutrition plan, and discharge focus. _(2026-07-25 incremental: the existing governed Today contract's optional schedule category, care location, discharge outlook, and patient-question fields now survive the release guard and arrive in both native apps. Schedule category is validated against the versioned patient-language registry; Android now decodes and renders the released care location, discharge-planning range/remaining steps, category, and questions rather than dropping them, while iOS renders the same category in the released plan detail and continues to use the released location/outlook. Android navigation resets each top-level care surface to its header, avoiding a deep offset inherited from another surface. The deterministic patient-only fixture, PHP projection tests, iPhone 17 Pro simulator, and Android API 35 emulator cover this behavior. This remains incomplete pending approved clinical content/source/release governance, all required Today fields, accessibility/usability review, and a pilot.)_
    - **2026-07-25 additional increment:** Today now composes a current care stage, released care-team summary/members, and released goals from the existing independently authorized Pathway and Care Team projections; it adds no duplicate Today field, source call, staff assignment, or inferred content. Each displayed card retains its projection provenance and is absent when the corresponding released projection is absent. The focused iPhone 17 Pro and Android API 35 journeys verify the composed cards. This is client presentation parity only, not an approved clinical source, release, pilot, or feature enablement.
...
- [x] Staff may acknowledge, answer, defer, or route it; every state and timestamp is patient-visible. _(2026-07-25: the pre-existing capability- and responsibility-pool-gated Patient Communications workflow is the single accountable lifecycle: staff claim to acknowledge, send a patient-visible reply to answer, and use the governed reroute flow to find the responsible team. The patient API now projects the authoritative latest receipt time as `state_updated_at`, which both native apps render beside the patient-visible delivery state. A promoted rounds question can additionally be deferred exactly once through the authorized staff rounds workspace: a content-free, append-only `round_question_promotion_deferrals` fact and generic encrypted status keep the question open for later review without exposing internal deliberation, an assignee, routing rationale, or a timing promise. The staff workspace links to the existing accountable queue for acknowledge/reply/reroute rather than duplicating it. Exact replay, distinct-repeat conflict, active grant/policy/retraction checks, patient-safe serialization, iOS Simulator, and Android API 35 emulator coverage all pass; flags remain default-off.)_
- [ ] After rounds, the patient sees an approved plain-language summary and updated goals/tasks.
    - **2026-07-25 additional increment:** Today now reuses the existing independently authorized `rounds_summary` patient projection when it is present. Both native views show its patient-facing headline, summary, approximate discussion window, released topics, provenance, and released next steps/questions beside the current care stage, team, and goals; no new request, source, route, schema, staff-rounds access, or inferred content was added. If the distinct release is missing, retracted, or feature-disabled, the card is absent rather than synthesized. iPhone 17 Pro and Android API 35 rendered journeys verify the behavior. This remains open pending an approved production source, review/release workflow, clinical/patient-advisor validation, pilot, and activation authorization.
- [x] Teach-back prompts record understanding or a request for clarification, not clinical consent unless a separate approved consent flow is used. _(2026-07-25: the patient path offers only an encrypted, accountable request for an explanation of a released education item. Its request schema and persisted association deliberately have no comprehension, completion, consent, order, care-plan, or clinician-assessment field; both native surfaces state that boundary before send. After a successful request, both apps bring the patient to the existing guarded Messages surface so the new conversation and its patient-visible status can be found; Android preserves an intentional navigation change made while the request was in flight. Unreleased/fabricated education fails closed and the reserved internal topic cannot be composed through the generic messaging endpoint.)_
...
- [ ] Status distinguishes scheduled, requested, waiting, transport requested, in progress, completed, result pending, and result released. _(2026-07-25 incremental: the coordinated `patient-state-vocabulary.v2-draft` contract now carries patient-safe English labels for all eight distinctions across the backend guard, OpenAPI, deterministic fixtures, iOS, and Android. `result_pending` deliberately contains no result content and renders only “Result not available yet.” This remains unchecked until an approved source/release-policy workflow can safely emit live clinical status and result availability.)_
- [ ] Delays use plain language and avoid exposing staffing/capacity details or an unreliable exact ETA.
    - **2026-07-25 incremental:** When a released schedule item is coded `delayed`, both native mappers now suppress its source-provided time window, detail, preparation, and timing-confidence text. They show only **Timing is being updated** and **The timing for this step has changed. Your care team will explain what happens next.** iOS gives `delayed` precedence over every upstream timing-confidence value so it remains “Being clarified.” Deterministic mapper inputs prove that a source-specific time and detail do not reach either patient snapshot; the iPhone 17 Pro and Android API 35 journeys verify the fixed wording. This remains unchecked until approved source/release governance, clinical-content review, and pilot validation exist.
...
- [x] Patient sees sent, delivered, assigned/acknowledged, responded, closed, and rerouted states. _(2026-07-25: the patient BFF already serializes only patient-safe thread ownership and message-delivery states; iOS decoded each contract state as a closed enum, while Android now renders the same patient-language meaning for `awaiting_team`, `assigned`, `acknowledged`, `responded`, `rerouted`, `escalated`, and `closed`, plus the corresponding delivery receipts. Neither client discloses a named assignee, responsibility pool, routing reason, staffing condition, or escalation implementation detail. Android displays the neutral **Status being confirmed** for an unexpected string rather than inventing a meaning. Unit and API-35 rendered regression coverage exercise the patient-language mapping. This is a default-off presentation control, not a messaging activation, response-time guarantee, or pilot authorization.)_
...
- [ ] Patient/family goals and preferences are visible to the care team. _(2026-07-25 partial: a patient can select a separately policy-released **What matters to you** preference or **A personal goal for my stay** topic in Messages. When the corresponding default-off feature, approved policy, active grant, and accountable handoff are available, the submission is an encrypted routed message to the internal care-team inbox; the optional preference/goal association is content-free and append-only. It cannot amend a clinical goal, care plan, order, consent, or assessment, and patient responses never expose the responsibility-pool identifier. My Path explains this boundary and now provides a patient-controlled **Open Messages** handoff; it changes only the native tab, never selects a topic/recipient or creates a draft/message. The iPhone 17 Pro topic-and-handoff tests, Android API 35 message-topic-and-handoff journey, and the PHP messaging suite cover it. This stays open because no representative/family delegation, approved production policy/source, staff-review workflow validation, pilot, or activation authority exists.)_
- [ ] Teach-back identifies areas needing clarification. _(2026-07-25 partial: a patient can author an encrypted, source-bound clarification request for one released education item, and both apps now take the patient to its existing Messages state after a successful send so the resulting conversation can be followed. The immutable association identifies only the released education item; it contains no comprehension score, completion, clinical assessment, or consent. This remains open because an approved clinical teach-back review/response workflow, language/interpreter accommodation, and patient-advisor validation have not been implemented.)_
...
- [ ] Every patient-facing state has a stable code and approved plain-language representation. _(2026-07-22 partial: `config/hummingbird-patient-content.php` now contains a versioned, default-English state registry, and `PatientProjectionContentGuard` rejects unknown schedule, pathway, milestone, event, goal, timing-confidence, discharge-criterion, rounds-topic, location, and contact-route codes before release. Both native apps map the currently accepted patient-visible state codes to matching context-specific English rather than title-casing an internal identifier; an unknown code falls back to “Status being confirmed.” Pathway-event category and governed urgent-contact wording now match across platforms, including the contract-only `call_button_for_urgent_help` value. Every patient envelope now has an additive `meta.state_vocabulary_version`, whose OpenAPI shape is checked in CI. An absent version remains compatible with an older server, but each native app withholds a projection that explicitly declares an incompatible version rather than render unapproved wording. The four-test, 12-assertion no-database registry/guard/metadata suite, iPhone 17 Pro unit/reference journey, and Android JVM/API 35 reference journey pass. This is a versioned English state-vocabulary foundation, not translated content, multidisciplinary content approval, or a complete patient-content registry.)_
    - **2026-07-25 incremental:** The coordinated `patient-state-vocabulary.v2-draft` update adds the patient-safe schedule labels `Scheduled`, `Waiting`, `Transportation requested`, `Result not available yet`, and `Result available` while retaining the existing codes. The release guard accepts only this versioned whitelist; iOS and Android withhold an explicitly incompatible envelope; deterministic BFF and device-test data exercise `result_pending` without a result payload. This does not approve a localized, clinically reviewed, or live-source content registry.
...
- [x] Add a Zephyrus web patient-communications inbox with facility/unit/service filters. _(2026-07-24: the capability-gated, content-free workspace now provides facility, unit, service-line, and responsible-team filters from the authorized open queue. Facility/service-line labels derive only from the active encounter's governed unit → facility-space → active facility/current primary service-line mapping; they never reuse a pool's broader routing scope and fail closed to `null` when the canonical registry is absent, inactive, expired, malformed, or unbounded. The staff OpenAPI schema, PHP contract guard, web Vitest coverage, API feature coverage, Android decoder/JVM suite plus API 35 emulator run, and iOS decoder/92-test simulator unit suite cover the additive projection. Push, pilot telemetry, and deployed E2E remain separate open release gates.)_
...
- [x] Add mobile thread read/reply/route/close actions for authorized staff. _(2026-07-21: the nine-operation BFF and both native clients implement inbox/detail/route-candidates/claim/reply/close/release/reassign/reroute with capability, membership, destination-responder, current-scope, no-store, exact-replay, optimistic-version, omission/access-loss/retained-transition purge, foreground lifecycle/lock polling gates, and no-blind-retry controls. 2026-07-24 incremental: each work item now carries server-computed `actions.can_claim`, `can_reply`, and `can_close`, derived from the current role, `mobile:act` token, effective pool membership permission, scope, ownership, and state; clients fail closed when that object is absent or malformed and never reconstruct it from an ownership-state heuristic. Mutations still re-evaluate authority under lock. The PHP feature suite (19 tests / 446 assertions), web Vitest (14 tests), iPhone 17 Pro simulator (93 unit tests plus the direct-action-denied UI journey), and API 35 Android emulator (12 communications journeys) pass. Automated shift/unit/discharge/pool-downtime reconciliation is local-only behind feature gates; authoritative service-change routing and deployed E2E remain separate backlog.)_
...
- [x] Build `capability-ledger.v1.yaml` and its generated human-readable coverage report. _(2026-07-24: the checked ledger owns 52 capabilities, 101 navigation routes, 60 staff mobile/auth operations, all nine staff communication operations, both staff session operations, and all 25 patient operations, including the two default-off notification-device operations, the source-bound education-clarification operation, and the feature-gated synthetic care-pathway journey demo.)_
...
- [ ] Establish generated/shared contracts and three-language fixture CI. _(2026-07-25 partial: PHP, iOS, and Android now ratify all eight server-factory staff BFF fixtures and all six test-only patient-projection BFF fixtures. Swift/Kotlin DTOs remain manually maintained; the generator, version pin, input-hash/output-freshness controls, and approved-source/release fixture lane have not landed. Generated/shared-contract completion is therefore still open.)_
...
- [ ] Close the remaining Android Eddy lifecycle and deployment gaps.
    - [x] Add the default-no-cache Android scoped chat turn from the authorized Eddy-context screen, including persona/screen/scope context, 503 unavailable-notice decoding, JVM contract decoding, and BFF request-shape evidence. This is deliberately not a claim of streaming, history, deletion, approval, or autonomous-action support.
    - [x] Add authorized, server-owned, no-store read-only conversation history to both native clients. _(2026-07-24: list/detail routes are user-scoped and exact-schema documented; Android exposes history from the authorized context chat and iOS exposes it from the chat toolbar. Neither client persists transcripts or queues an Eddy request offline. The focused BFF suite passes 8 tests/53 assertions; Android JVM and full API 35 emulator instrumentation pass, including two history UI tests; iPhone 17 Pro Simulator tests prove no-store transport for context, chat, list, and detail plus safe decoding of null draft actions. Streaming and deletion remain open.)_
    - [x] Add the existing server-governed Eddy approval inbox, fetch-on-open dry run, and explicit online human approve/reject decision to both native clients. _(2026-07-24: Android opens the no-store queue from its authorized Eddy context; iOS opens it from the Eddy chat toolbar. Both retain only transient view state, require a confirmation step, pass the selected persona and an in-memory exact idempotency key, and never queue/replay a mutation offline. The server independently enforces `mobile:act`, user scope, current persona, pending state, and exact replay. Android JVM/API 35 and iPhone 17 Pro transport tests cover no-store, persona, explicit idempotency, selected-record navigation, and visible human/online boundary; BFF coverage retains read-only-token rejection and allowed human decision evidence. Streaming, deletion, role-by-role live deployment evidence, and autonomous action remain open.)_
    - [x] Add Android’s persistent signed-in Eddy entry without widening the chat context. _(2026-07-24: the app-shell affordance opens the existing server-authorized `house` lens only; it neither derives a patient scope from the visible UI nor carries patient-communications state into Eddy. The Android API 35 UI test verifies the visible global entry and its bounded house-lens delegate. Active-screen context remains iOS-led, while streaming and history deletion remain open.)_
    - [x] Add bounded no-store SSE chat to both native clients. _(2026-07-24: the mobile BFF now parses the upstream stream and emits only a `conversation_id`, safe token frames, a server-persisted clean terminal reply, a separately sanitized persisted proposal, and `[DONE]`; it does not relay the raw upstream terminal proposal frame. iOS uses `URLSession.AsyncBytes`; Android uses a cancellation-aware `HttpURLConnection` flow. Both send no-store headers, impose a 45-second inactivity bound, retain no transcript, omit idempotency/replay, render provisional text in one pending bubble, and suppress partial proposal markup as defense in depth. The focused Laravel BFF suite passes 9 tests/66 assertions and stream service suite passes 3/13; Android passes 122 Debug JVM tests and 26 API 35 AVD instrumentation tests; iPhone 17 Pro Simulator passes 110 tests. The native transport/parser tests cover headers, no replay, terminal handling, and proposal isolation. History deletion, role-by-role deployed evidence, and autonomous action remain open.)_
    - [ ] Add server-authorized conversation deletion only after retention/audit policy is approved. _(There is no mobile deletion operation today; neither client may simulate local deletion of the server-owned transcript.)_
...
- [x] Add a dry-run-first, idempotent synthetic reference-inpatient provisioner and verify it against the canonical backend. _(2026-07-20: `hummingbird:seed-reference-patient` established exactly one active canonical row for encounter `10040`, unit `85`. Reconciled 2026-07-24: an exact guarded transaction preserved that sole command-owned, active, not-discharged row in unit `85` and set its expected-discharge date to `2026-07-26`; the separately provisioned patient identity and grant remain pending/inactive, with no session or token. 2026-07-25: the preview path was corrected to derive, but never issue, its opaque context reference; a local regression proves it leaves both encounter and resolver cache unchanged, and the remote read-only guard returned encounter `10040` with `patient_context_ref_issued=false`. See §3.2.8.)_
...
- [ ] Create and approve a patient projection for the synthetic encounter. _(2026-07-24 partial: the guarded draft command is deployed at release `f7074936a2f265fbfd785f69c5f30632503e2fe8`. Its production dry run, exact commit, and deterministic replay created and revalidated six synthetic, non-patient-visible drafts for Today, My Path/pathway events, discharge readiness, rounds summary, and Care Team. An independent direct database audit found exactly six drafts/cursors, one unapproved draft policy, zero released projections/publication facts, pending/inactive access, and zero sessions/tokens; every patient exposure gate remains disabled. Independent clinical/content approval and independent release authority remain deliberately open, so this combined item cannot be checked. See §3.2.8.)_
...
- [ ] Implement production source reconciliation, projection workers, lag measurement/alerting, replay, and dead-letter recovery. _(2026-07-22 partial: `PatientPathwaySourceReconciliationService` provides a default-off, no-route, configuration-allowlisted ingestion boundary with all-or-nothing append/replay behavior; `PatientPathwaySourceReconciliationTest` covers approved admission, duplicate replay, absence-without-cancellation, unapproved-source/gate rejection, and rollback. The separate `PatientPathwayProjectionReviewReleaseService` persists two-person clinical approval/release execution and atomically publishes a reviewed release through the existing outbox. **2026-07-24 incremental:** the existing one-minute, single-server, non-overlapping `admin:observe-system-health` collector includes a required `patient_projection_pipeline` component. When all four draft-pathway gates are intentionally enabled, it reads only aggregate active-grant instance counts, observation ages, expected/missing/source-behind latest-draft counts, freshness classes, and recent retryable/manual-review/terminal failures. It has no patient route, source call, cursor mutation, projection content access, release action, or feature activation. Missing observation, missing/behind draft, stale draft, critical lag, or repeated recent failures produce one deduplicated critical system-health alert; disabled governance gates are explicitly healthy and partial gates warn. The new [runbook](../operations/HUMMINGBIRD-PATIENT-PROJECTION-PIPELINE-RUNBOOK.md) forbids direct SQL or ungoverned replay. The monitor suite is PHI-free and verifies disabled/partial gates, missing and source-behind drafts, current/stale drafts, repeated failures, and critical-alert evidence. No source is allowlisted in shipped configuration, and no deployed connector, production source/review workflow, dead-letter recovery procedure, or patient activation exists.)_
    - [x] Add a PHI-free pathway-projection readiness monitor. _(2026-07-24: the required patient projection system-health component activates only after all four draft-pathway governance gates are intentionally enabled. It reads aggregate active-grant instance counts, observation ages, expected/missing/source-behind latest-draft counts, freshness classes, and recent retryable/manual-review/terminal failures. It never calls a source, mutates a cursor, reads projection content, releases content, or enables a feature. Its seven-case suite covers disabled and partial gates, a missing draft, a draft that lags newer source history, a current matching draft, stale draft freshness, repeated failures, PHI-free details, and the deduplicated critical alert.)_
...
- [x] Complete the governed Today schedule-field contract across the release guard and both native clients. _(2026-07-25: the OpenAPI-documented `PatientScheduleItem.category` was previously rejected by the backend content guard and silently dropped by native decoding. It is now admitted only as the patient-language registry's `test`/`procedure`/`transport`/`other` category, and both apps render a typed generic fallback rather than a raw future code. Android also carries the existing optional `care_location`, `discharge_outlook`, and `questions` fields into its Today view, including a released location card, an uncertainty-labelled discharge-planning item, and patient questions alongside next steps. A deterministic fixture is regenerated from the actual patient BFF response; PHP guard/API/fixture tests, iPhone 17 Pro simulator tests, Android JVM tests, and the API 35 emulator pass. This is projection-contract parity only—not source approval, a clinical release, results delivery, an exact ETA, a care-plan mutation, or patient-feature activation.)_
- [ ] Add education assignments and governed teach-back review. _(2026-07-21 partial, navigation evidence refreshed 2026-07-25: released education has its own My Path section on iOS and Android. A successful source-bound clarification request now takes a patient who remains on My Path to Messages, where the returned conversation and patient-visible state are available; Android deliberately preserves a different patient-selected destination while the request is in flight. Assignment, clinical review, comprehension assessment, education completion, consent, and outcome workflows remain unimplemented.)_
- [x] Allow a source-bound education clarification request without asserting comprehension. _(2026-07-21, post-send navigation verified 2026-07-25: a patient may use the default-off `POST /encounters/{encounterUuid}/education/{educationItemUuid}/clarifications` endpoint only when messaging and education-clarification flags are enabled and the item UUID is contained in the patient's current released pathway projection. The server creates the existing encrypted, accountable care-team message under a reserved policy topic and a separate append-only, content-free association fact; generic messaging cannot enumerate or compose the reserved topic. The request is idempotent, no-store, audit-recorded, and returns only the ordinary message thread. It never accepts or stores a completion, comprehension, consent, clinician-assessment, care-plan, or order field. iOS and Android render it as “Ask for an explanation,” keep it out of synthetic/offline mode, and send only the message, client UUID, urgent-guidance version, and normal idempotency key. On success, both present the existing guarded Messages surface rather than leaving the new conversation undiscoverable; Android does not override a different navigation choice made before the request returns. Focused iOS Simulator, Android JVM, and API 35 emulator checks pass; the PHP feature test is present but cannot boot locally while the PostgreSQL maintenance service is unavailable.)_
...
- [x] Route/OpenAPI exact parity for staff and patient groups. _(2026-07-23: staff inventory is exact across 60 authenticated operations and 60 OpenAPI operations, including all nine staff communication and both staff session operations; patient inventory is exact across all 25 operations, including the default-off notification-device registration/revocation pair and the source-bound education-clarification operation; all local staff-contract references resolve.)_
...
- [ ] Projection provenance, release, sensitivity, correction, retraction, and lag tests. _(2026-07-24 partial: `PatientProjectionPipelineHealthTest` creates version-pinned history through the default-off reconciliation boundary, proves aggregate-only missing/source-behind/stale freshness evidence plus repeated-failure alerting, and verifies that opaque assignment/event references cannot appear in health details. Full approved-source, release, correction/retraction, and deployed lag coverage remain.)_
    - [x] Add aggregate missing-draft, source-behind, stale-freshness, and repeated-failure monitoring tests. _(2026-07-24: the monitor suite proves no source identifier reaches health details or alerts; full approved-source, clinical release, correction/retraction, and deployed lag coverage remain open.)_
