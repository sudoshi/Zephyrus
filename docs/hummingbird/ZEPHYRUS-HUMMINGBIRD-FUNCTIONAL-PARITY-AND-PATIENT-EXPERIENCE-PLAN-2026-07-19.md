...
- [ ] Deploy this draft-projection implementation through the clean synchronized `main` checkout and canonical `./deploy.sh --check` then `./deploy.sh` procedure. On the deployed runtime, dry-run, commit, and exact-replay only encounter `10040`; then independently audit counts only: six drafts, zero released projections, zero matching publication facts, draft policy with no approver/effective window, pending principal/grant, and zero sessions/tokens. This item remains open until those production facts are observed and recorded.
...
- [ ] Create and approve a patient projection for the synthetic encounter. _(2026-07-24 partial: a guarded dry-run-first command and fail-closed regression suite now support six deterministic, synthetic, non-patient-visible drafts for Today, My Path/pathway events, discharge readiness, rounds summary, and Care Team. At this checkpoint the implementation is not yet deployed and production still has zero projections. After canonical deployment, the engineering fixture may create drafts only; independent approval and release remain deliberately open, so this combined item cannot be checked. See §3.2.8.)_
