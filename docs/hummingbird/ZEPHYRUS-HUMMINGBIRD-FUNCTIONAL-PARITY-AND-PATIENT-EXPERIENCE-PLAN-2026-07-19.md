...
#### 3.2.5 Staff token-rotation parity and emulator evidence — 2026-07-23
...
- [x] The Laravel refresh endpoint now locks the exact presented refresh-token row inside the rotation transaction, deletes that one-time predecessor, and issues one successor pair. Feature coverage proves the successor access token works and the predecessor cannot rotate again. This closes same-token concurrent double issuance; refresh-token family identity, reuse-theft detection, and family-wide revocation remain explicitly open.
- [x] Device and build evidence is green with no skips: iOS **70/70 unit**, **16/16 UI** on an iPhone 17 Pro simulator, and optimized Release build; Android **100/100 Debug JVM**, **97/97 Release JVM**, lint, Debug/Release assembly, and **18/18** instrumentation tests on an API 35 emulator. Focused cross-platform tests cover near-expiry writes, one-time GET replay, rejected-replay invalidation, mutation non-replay, eight-caller single flight, transient fallback and bootstrap retention, terminal refresh rejection, rotated-pair persistence, protected-store failure, cleared-generation non-resurrection, and Android Keystore-backed migration. A repeated iOS UI pass exposed a fixture race in which a scheduled authorization purge removed a navigation bar between accessibility lookup and a non-semantic tap; the tap was removed, the focused journey passed, and the complete 16-test suite then passed from a clean rerun.
...
- [ ] Rotate refresh tokens on use; revoke the predecessor; detect reuse; revoke the token family on suspected theft. _(2026-07-23 partial: the server serializes consumption of the exact presented token, deletes the predecessor, and returns a rotated pair. Stable family identity, predecessor-reuse theft detection after deletion, and family-wide revocation remain unimplemented, so this broader item stays open.)_
