...
| Eddy staff             | Chat, stream, conversations, approvals, context                                                                                                       | Chat, context, no-store read-only history, and governed approval review/decision | Global authorized-house and scoped nonstreaming chat, no-store read-only history, and governed approval review/decision | `PARTIAL`: history and approvals are server-scoped/no-store on both clients; neither client has streaming or history deletion, and Android’s global entry deliberately exposes no active-screen or patient-message context |
...
| Eddy chat/conversations                           | Both native clients have authorized, no-store read-only history; Android has a global authorized-house entry plus scoped nonstreaming chat        | `NATIVE`                      | P1: streaming fallback, active-screen context parity, conversation retention/deletion policy |
