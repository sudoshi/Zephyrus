...
- [ ] Split broad and unit/task-level patient access policies into named policy classes with audit reason codes. _(2026-07-22 implementation pending CI execution evidence: `MobilePatientContextAuthorizationService` now evaluates separately named broad-persona, house-operations, active-transport, active-EVS, and shared-active-unit policies on every staff disclosure; it rejects inactive accounts and stale personas, writes content-free opaque-handle audit decisions with machine reason codes, and fails closed if a successful-disclosure audit cannot persist. `MobileBackendSafetyTest::test_patient_context_authorization_uses_named_scope_policy_and_safe_audit_reason_codes` covers allowed/denied scope and non-disclosure. The item remains open until the isolated PostgreSQL gate runs.)_
