...
3. The requested Hummingbird persona is still authorized for that user.
4. The handle resolves and the patient still has a current operational context: a pending bed request, active transport/EVS task, nondeparted ED visit, active inpatient encounter, or active-status perioperative case. Terminal or historical workflow rows do not keep a context current.
...
| `shared_active_unit`       | Charge nurse, bedside nurse, hospitalist, intensivist | A live `prod.user_unit` assignment intersects an active encounter unit, nondeparted ED unit, or nonterminal EVS unit | `shared_active_unit_authorized`       | `patient_unit_assignment_unavailable` or `shared_active_unit_required` |
...
Success is fail-closed: after constructing the entire context, the service commits the cache write and durable success audit as one database transaction. It returns no context if either write fails, and an audit outage cannot leave a populated unaudited cache. A construction failure therefore never creates a false successful-disclosure event. Denials use the recorder's best-effort path so an unavailable audit sink cannot become a patient-context existence oracle.
