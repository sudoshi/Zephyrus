...
## Read the path references with one substitution

These documents were written for an effort (PR #92) that built Nightingale as a new,
independent application tree at `nightingale/iosApp` and `nightingale/androidApp`. That
effort was superseded: the shipping app was instead produced by renaming the existing
patient application in place (PR #101), which was already feature-complete and is what
reaches TestFlight today.

The governance, threat-model, accessibility, privacy, and identity decisions recorded here
all still stand. Only the file locations moved:

| Written as | Read as |
| --- | --- |
| `nightingale/iosApp/Nightingale/…` | `hummingbird/iosPatientApp/Nightingale/…` |
| `nightingale/androidApp/app/src/…` | `hummingbird/androidPatientApp/app/src/…` |

The two `hummingbird/…PatientApp` directory names are deliberate: they describe platform
and role, not brand. Nothing patient-visible carries the Hummingbird name — bundle
identifier, application ID, display name, and all user-facing copy are Nightingale.

The `App\Nightingale\Activation` and `App\Nightingale\Disclosure` gates, the foundation
keys in `config/nightingale.php`, and `PrivacyInfo.xcprivacy` were carried across. The two
scaffolds themselves were not: they contained no features and no networking, so adopting
them would have replaced a working patient application with a launch screen.
