...
|  11 | `GET /encounters` (`listPatientEncounterAccess`)                                                                         | Principal-owned effective grants, policy, opaque grant/encounter UUIDs, audit                                      | **First read candidate.** Requires Nightingale relationship/purpose/facility policy and non-disclosure tests.                                          |
|  12 | `GET /encounters/{encounterUuid}/today` (`getPatientTodayProjection`)                                                    | `today:read`, grant policy, effective release policy, released projection, relationship allowlist, content actions | **Core read candidate.** Field matrix, freshness/staleness, correction/retraction, content and patient-language approval remain open.                  |
