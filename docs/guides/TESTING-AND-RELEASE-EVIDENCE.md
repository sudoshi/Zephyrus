...
List the lanes with `bash scripts/test-suite.sh list`. `scripts/run-browser-suite.sh` creates a random `zephyrus_test_e2e*` database through the loopback-only `scripts/manage-test-database.php` guard, seeds the guarded browser actor and deterministic synthetic cross-domain release fixture, starts a directly owned loopback PHP server process, runs Chromium, then terminates the server and drops the database. The fixture uses one immutable clock and covers RTDC, Perioperative, Cockpit, Laboratory, Radiology, and Pharmacy without production PHI. Each invocation also creates a private per-run secret root and encrypted clinical-payload root below `storage/framework/testing/browser`, writes a random test-only KEK with mode `0600`, and removes the entire resource root during cleanup. Cleanup failure makes the lane fail. It must never target production.

`npm test` and `npm run build` both run `npm run icons:check` first. The repository-owned generator scans static Iconify references for the allowlisted Carbon, Healthicons, Heroicons, Lucide, MDI, and Solar prefixes and verifies that `resources/js/iconify-bundle.ts` is complete and current. Runtime icons are registered from that local bundle, so application CSP does not depend on the public Iconify API; missing or drifted icons fail the gate instead of becoming an external production request. Realtime initialization is separately bounded to configured same-host deployments unless an explicit cross-origin opt-in is supplied.
