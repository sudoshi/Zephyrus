...
/** Dedicated, version-addressable encryption boundary for provider tokens. */
final class PatientNotificationDeviceCipher
{
    ...
    public function encrypt(string $token, string $keyVersion, string $context): string
    {
        ...
        try {
            ...
            throw new RuntimeException('patient_notification_device_encryption_envelope_invalid', previous: $exception);
        }
    ...
    public function decrypt(string $ciphertext, string $keyVersion, string $context): string
    {
        ...
        if (! is_array($envelope)
            ...
            throw new RuntimeException('patient_notification_device_encryption_context_mismatch');
        }

        return $envelope['token'];
    }

    public function contextFor(string $deviceUuid): string
    {
        if (! Str::isUuid($deviceUuid)) {
            throw new RuntimeException('patient_notification_device_encryption_context_invalid');
        }

        return 'notification-device:'.Str::lower($deviceUuid);
    }
    ...
    private function encrypter(string $keyVersion): Encrypter
    {
        ...
        if (! Encrypter::supported($key, 'AES-256-CBC')) {
            throw new RuntimeException('patient_notification_device_encryption_key_invalid');
        }
    ...
    private function keyForVersion(string $keyVersion): string
    {
        ...
        if ($this->app->environment('testing') && str_starts_with($keyVersion, 'test-')) {
            return hash('sha256', 'hummingbird-patient-notification-device-test-only-key|'.$keyVersion, true);
        }

        throw new RuntimeException('patient_notification_device_encryption_key_unavailable');
    }
    ...
    private function previousKey(string $keyVersion): mixed
    {
        ...
        try {
            ...
            throw new RuntimeException('patient_notification_device_encryption_key_ring_invalid', previous: $exception);
        }

        if (! is_array($keyRing)) {
            throw new RuntimeException('patient_notification_device_encryption_key_ring_invalid');
        }
    ...
    private function decodeKey(string $configured): string
    {
        if (! str_starts_with($configured, 'base64:')) {
            throw new RuntimeException('patient_notification_device_encryption_key_format_invalid');
        }
    ...
    private function contextDigest(string $context): string
    {
        ...
        return hash('sha256', "hummingbird-patient-notification-device\0".$context);
    }
}
