...
{
    /**
     * Get the saved layout for a user's process view.
     */
    public function getSavedLayout(int $userId, string $hospital, string $workflow, string $timeRange): ?array
    {
        ...
        return $layout ? $layout->layout_data : null;
    }

    /**
     * Build the full nursing operations response data for a given workflow and time range.
     */
    public function getNursingOperations(string $hospital, string $workflow, string $timeRange): array
    {
        $data = $this->getNursingOperationsData($workflow);

        $data['metrics'] = $this->buildIntelligenceMetrics($workflow);

        $multiplier = $this->getTimeRangeMultiplier($timeRange);

        $data = $this->applyMultiplierToData($data, $multiplier);

        return $data;
    }

    /**
     * Build intelligence metrics for a workflow.
     */
    private function buildIntelligenceMetrics(string $workflow): array
    {
        return [
            'staffing' => [
                'nurses' => [
     ...
     */
    private function getTimeRangeMultiplier(string $timeRange): int
    {
    ...
    /**
     * Apply a multiplier to node counts and edge patient counts.
     */
    private function applyMultiplierToData(array $data, int $multiplier): array
    {
        $nodes = $data['nodes'];
        foreach ($nodes as $index => $node) {
            if (isset($node['data']['metrics']['count'])) {
                $nodes[$index]['data']['metrics']['count'] = $node['data']['metrics']['count'] * $multiplier;
            }
        }
        $data['nodes'] = $nodes;
        ...
        foreach ($edges as $index => $edge) {
            if (isset($edge['data']['patientCount'])) {
                $edges[$index]['data']['patientCount'] = $edge['data']['patientCount'] * $multiplier;
            }
        }
        $data['edges'] = $edges;
    ...
    /**
     * Generate nursing operations data based on workflow type.
     */
    private function getNursingOperationsData(string $workflow = 'Admissions'): array
    {
        if ($workflow === 'Discharges') {
            return $this->getDischargeWorkflowData();
        }

        return $this->getAdmissionsWorkflowData();
    }

    /**
     * Get discharge workflow nodes and edges.
     */
    private function getDischargeWorkflowData(): array
    {
        return [
            'nodes' => [
                // Clinical Branch
    ...
    /**
     * Get admissions workflow nodes and edges.
     */
    private function getAdmissionsWorkflowData(): array
    {
        return [
            'metrics' => [
                'staffing' => [
